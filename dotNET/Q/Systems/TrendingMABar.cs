...
namespace Q.Systems {
    ...
    public class TrendingMABar : TrendingMABase {
        public TrendingMABar(QREBridgeBase bridge, Symbol symbol) : base(bridge, symbol, priceSeries) {}
//      (high + low + close) / 3 
        ...
        protected override void placeEntries() {
            ...
            info(signal.last10());
            placeOrder(symbol.buy("Upper Range Break", stop(entryUp), tradeSize(), oneBar()));
            placeOrder(symbol.sell("Lower Range Break", stop(entryDn), tradeSize(), oneBar()));
        }

        protected override void placeExits() {
            var reversalExit = Math.Round(maDays * (upperBand + lowerBand) / 2 - shortSum, 4);
            info(signal.last10());
            placeOrder(position().exit("Exit " + lOrS() + " Reversal", stop(reversalExit), oneBar()));
        }
    }        
}
