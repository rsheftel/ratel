...
namespace Q.Systems {
    public class TrendingMAPriceClose : TrendingMABase {
        public TrendingMAPriceClose(QREBridgeBase bridge, Symbol symbol) : base(bridge, symbol, bars => bars.close) {}
        ...
        protected override void placeEntries() {
            if (ma > upperBand) placeOrder(symbol.buy("Upper Range Break", protectiveStop(signal), tradeSize(), oneBar()));
            if (ma < lowerBand) placeOrder(symbol.sell("Lower Range Break", protectiveStop(signal), tradeSize(), oneBar()));       
        }
        ...
        protected override void placeExits() {
            ...
            if ((ma - reversalExit) * position().direction() < 0) 
                placeOrder(position().exit("Exit " + lOrS() + " Reversal", protectiveStop(signal), oneBar()));
        }
