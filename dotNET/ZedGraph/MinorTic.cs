...
namespace ZedGraph {
    ...
    [Serializable] public class MinorTic : ICloneable, ISerializable {
        ...
        internal void Draw(
            ...
            if (IsOutside) g.DrawLine(pen, pixVal, shift, pixVal, shift + scaledTic);

            // draw the cross tic
            if (_isCrossOutside) g.DrawLine(pen, pixVal, 0.0f, pixVal, scaledTic);

            // draw the inside tic
            if (IsInside) g.DrawLine(pen, pixVal, shift, pixVal, shift - scaledTic);

            // draw the inside cross tic
            if (_isCrossInside) g.DrawLine(pen, pixVal, 0.0f, pixVal, -scaledTic);

            // draw the opposite tic
            if (IsOpposite) g.DrawLine(pen, pixVal, topPix, pixVal, topPix + scaledTic);
        }
