...
*/
        ...
        static bool CheckValidDate(double xlDate) {
        ...
        public static double MakeValidDate(double xlDate) {
            if (xlDate < XLDayMin) xlDate = XLDayMin;
            if (xlDate > XLDayMax) xlDate = XLDayMax;
            return xlDate;
        }
        ...
        public static double CalendarDateToXLDate(
            ...
            // setting hour to 24 would be the first hour of the next day, etc.
            var sec = (int) second;
            var ms = (second - sec) * MillisecondsPerSecond;
            NormalizeCalendarDate(
                ref year,
                ref month,
                ref day,
        ...
        /// </param>
        static void NormalizeCalendarDate(
            ref int year,
            ref int month,
            ref int day,
            ref int hour,
            ...
            var carry = (int) Math.Floor(millisecond / MillisecondsPerSecond);
        ...
        /// </param>
        /// <returns>The corresponding XL date, expressed in double floating point format</returns>
        static double _CalendarDateToXLDate(
            int year,
            int month,
            int day,
            int hour,
            ...
            int second,
            double millisecond) {
            return JulianDayToXLDate(
                _CalendarDateToJulianDay(
                    year,
                    month,
                    day,
                    hour,
        ...
        /// <returns>The corresponding Astronomical Julian Day number, expressed in double
        /// floating point format</returns>
        static double _CalendarDateToJulianDay(
            int year,
            int month,
            int day,
            int hour,
        ...
        public static void XLDateToCalendarDate(
            ...
            out int minute,
            out int second) {
            var jDay = XLDateToJulianDay(xlDate);
        ...
        public static void JulianDayToCalendarDate(
            ...
            var z = Math.Floor(jDay + 0.5);
            var f = jDay + 0.5 - z;

            var alpha = Math.Floor((z - 1867216.25) / 36524.25);
            var A = z + 1.0 + alpha - Math.Floor(alpha / 4);
            var B = A + 1524.0;
            ...
            var fday = (jDay - 0.5) - Math.Floor(jDay - 0.5);
        ...
        public static double XLDateToDecimalYear(double xlDate) {
            ...
            var jDay1 = CalendarDateToJulianDay(year, 1, 1, 0, 0, 0);
            var jDay2 = CalendarDateToJulianDay(year + 1, 1, 1, 0, 0, 0);
            var jDayMid = CalendarDateToJulianDay(year, month, day, hour, minute, second);
        ...
        public void AddMonths(double dMonths) {
            var iMon = (int) dMonths;
            var monFrac = Math.Abs(dMonths - iMon);
            var sMon = Math.Sign(dMonths);
            ...
            if (sMon == 0) return;
            var xlDate2 = CalendarDateToXLDate(year, month + sMon, day, hour, minute, second);
        ...
        public void AddYears(double dYears) {
            var iYear = (int) dYears;
            var yearFrac = Math.Abs(dYears - iYear);
            var sYear = Math.Sign(dYears);
            ...
            if (sYear == 0) return;
            var xlDate2 = CalendarDateToXLDate(year + sYear, month, day, hour, minute, second);
        ...
        public int CompareTo(object target) {
            if (! (target is XDate)) throw new ArgumentException();
        ...
        public override bool Equals(object obj) {
            if (obj is XDate) return ((XDate) obj)._xlDate == _xlDate;
            if (obj is double) return ((double) obj) == _xlDate;
            return false;
        }
        ...
        public static string ToString(ref XDate date, double xlDate) {
        ...
        public static string ToString(double xlDate, string fmtStr) {
            ...
            if (!CheckValidDate(xlDate)) return "Date Error";
            ...
            if (fmtStr.IndexOf("[f]") >= 0) fmtStr = fmtStr.Replace("[f]", ((int) (xlDate * 864000)).ToString("d"));
            if (fmtStr.IndexOf("[ff]") >= 0) fmtStr = fmtStr.Replace("[ff]", ((int) (xlDate * 8640000)).ToString("d"));
            if (fmtStr.IndexOf("[fff]") >= 0) fmtStr = fmtStr.Replace("[fff]", ((int) (xlDate * 86400000)).ToString("d"));
            if (fmtStr.IndexOf("[ffff]") >= 0) fmtStr = fmtStr.Replace("[ffff]", ((int) (xlDate * 864000000)).ToString("d"));
            if (fmtStr.IndexOf("[fffff]") >= 0) fmtStr = fmtStr.Replace("[fffff]", ((int) (xlDate * 8640000000)).ToString("d"));
