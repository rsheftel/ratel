...
namespace ZedGraph {
    ...
    [Serializable] public class PointPairList : List<PointPair>, IPointListEdit {
        #region Fields
...
*/
        ...
        public PointPairList(IPointList list) {
            ...
            for (var i = 0; i < count; i++) Add(list[i]);
        ...
        public void Add(PointPairList pointList) {
            foreach (var point in pointList) Add(point);
        ...
        public void Add(double[] x, double[] y) {
            ...
            if (y != null && y.Length > len) len = y.Length;

            for (var i = 0; i < len; i++) {
                var point = new PointPair(0, 0, 0);
                if (x == null) point.X = i + 1.0;
                else point.X = i < x.Length ? x[i] : PointPairBase.Missing;

                if (y == null) point.Y = i + 1.0;
                else point.Y = i < y.Length ? y[i] : PointPairBase.Missing;
        ...
        public void Add(double[] x, double[] y, double[] z) {
            ...
            if (x != null) len = x.Length;
            if (y != null && y.Length > len) len = y.Length;
            if (z != null && z.Length > len) len = z.Length;
            ...
            for (var i = 0; i < len; i++) {
                ...
                if (z == null) point.Z = i + 1.0;
                else point.Z = i < z.Length ? z[i] : PointPairBase.Missing;
		...
		*/
        ...
        public int IndexOfTag(string label) {
            ...
            foreach (var p in this) {
                if (p.Tag is string && String.Compare((string) p.Tag, label, true) == 0) return iPt;
                iPt++;
            }
        ...
        public override bool Equals(object obj) {
            ...
            if (rhs != null) {
                ...
                for (var i = 0; i < Count; i++) if (!this[i].Equals(rhs[i])) return false;
            }
        ...
        public void SetX(double[] x) {
            for (var i = 0; i < x.Length; i++) if (i < Count) this[i].X = x[i];
        ...
        public void SetY(double[] y) {
            for (var i = 0; i < y.Length; i++) if (i < Count) this[i].Y = y[i];
        ...
        public void SetZ(double[] z) {
            for (var i = 0; i < z.Length; i++) if (i < Count) this[i].Z = z[i];
        ...
        public void SumY(PointPairList sumList) {
            for (var i = 0; i < Count; i++) if (i < sumList.Count) this[i].Y += sumList[i].Y;
        ...
        public void SumX(PointPairList sumList) {
            for (var i = 0; i < Count; i++) if (i < sumList.Count) this[i].X += sumList[i].X;
        ...
        public double InterpolateX(double xTarget) {
            int lo;
            int hi;
            if (Count < 2) throw new Exception("Error: Not enough points in curve to interpolate");
            ...
            if (xTarget <= this[0].X) {
                ...
                for (j = 0; j < 1000 && hi > lo + 1; j++) {
                    var mid = (hi + lo) / 2;
                    if (xTarget > this[mid].X) lo = mid;
                    else hi = mid;
                }

                if (j >= 1000) throw new Exception("Error: Infinite loop in interpolation");
            }
        ...
        public double SplineInterpolateX(double xTarget, double tension) {
            ...
            if (xTarget <= this[0].X || xTarget >= this[Count - 1].X) return PointPairBase.Missing;
            // if x is within the bounds of the x table, then do a binary search
            // in the x table to find table entries that bound the x value
            var lo = 0;
            var hi = Count - 1;
            ...
            for (var t = 0.01; t <= 1; t += 0.01) {
                var B0 = (1 - t) * (1 - t) * (1 - t);
                var B1 = 3.0 * t * (1 - t) * (1 - t);
                var B2 = 3.0 * t * t * (1 - t);
                var B3 = t * t * t;

                var newX = X1 * B0 + (X1 + (X2 - X0) * tension) * B1 + (X2 - (X3 - X1) * tension) * B2 + X2 * B3;
                var newY = Y1 * B0 + (Y1 + (Y2 - Y0) * tension) * B1 + (Y2 - (Y3 - Y1) * tension) * B2 + Y2 * B3;

                // We are looking for the first X that exceeds the target
                if (newX >= xTarget) // We now have two bounding X values around our target
        ...
        public PointPairList LinearRegression(
            ...
            if (count < 2 || maxX - minX < 1e-20) return null;
