...
namespace ZedGraph {
    ...
    [Serializable] public class CurveList : List<CurveItem>, ICloneable {
        #region Properties
        ...
        public int NumBars {
            get {
                var count = 0;
                foreach (var curve in this) if (curve.IsBar) count++;
        ...
        public int NumClusterableBars {
            get {
                var count = 0;
                foreach (var curve in this) if (curve.IsBar || curve is HiLowBarItem) count++;
        ...
        public int NumPies {
            get {
                var count = 0;
                foreach (var curve in this) if (curve.IsPie) count++;
        ...
        public bool HasData() {
            foreach (var curve in this) if (curve.Points.Count > 0) return true;
            return false;
        }
        #endregion
        #region Constructors
        ...
        public CurveList(CurveList rhs) {
            ...
            foreach (var item in rhs) Add((CurveItem) ((ICloneable) item).Clone());
        }
        ...
        public IEnumerable<CurveItem> Backward {
            get { for (var i = Count - 1; i >= 0; i--) yield return this[i]; }
        }
        /// <summary>
        /// Iterate forward through the <see cref="CurveList" /> items.
        /// </summary>
        public IEnumerable<CurveItem> Forward {
            get { for (var i = 0; i < Count; i++) yield return this[i]; }
        }
        #endregion
        #region List Methods
/*
...
*/
        ...
        public int IndexOf(string label) {
            ...
            foreach (var p in this) {
                if (String.Compare(p._label._text, label, true) == 0) return index;
                index++;
            }
        ...
        public int IndexOfTag(string tag) {
            ...
            foreach (var p in this) {
                if (p.Tag is string &&
                    String.Compare((string) p.Tag, tag, true) == 0) return index;
                index++;
            }
        ...
        public void GetRange(bool bIgnoreInitial, bool isBoundedRanges, GraphPane pane) {
            ...
            foreach (var axis in pane.YAxisList) InitScale(axis.Scale, isBoundedRanges);

            foreach (var axis in pane.Y2AxisList) InitScale(axis.Scale, isBoundedRanges);
            ...
            foreach (var curve in this) {
                ...
                if (curve.IsBar)
                    if (pane._barSettings.Base == BarBase.X ||
                        ...
                        if (! (curve is HiLowBarItem))
                            if (tYMinVal > 0) tYMinVal = 0;
                            else if (tYMaxVal < 0) tYMaxVal = 0;
                        ...
                        // Only force z=0 for BarItems, not HiLowBarItems
                        if (!(curve is HiLowBarItem))
                            if (tXMinVal > 0) tXMinVal = 0;
                            else if (tXMaxVal < 0) tXMaxVal = 0;
                ...
                if (curve.NPts > MaxPts) MaxPts = curve.NPts;
            ...
            foreach (var axis in pane.YAxisList) axis.Scale.SetRange(pane, axis);
            foreach (var axis in pane.Y2AxisList) axis.Scale.SetRange(pane, axis);
        }

        static void InitScale(Scale scale, bool isBoundedRanges) {
        ...
        static void GetStackRange(
            ...
            for (var i = 0; i < curve.Points.Count; i++) {
                ...
                if (x == PointPairBase.Missing || y == PointPairBase.Missing || lowVal == PointPairBase.Missing) continue;
        ...
        public void Draw(Graphics g, GraphPane pane, float scaleFactor) {
            ...
            if (pane._barSettings.Type == BarType.SortedOverlay) {
                ...
                foreach (var curve in this) if (curve.IsBar) tempList.Add(curve);
            ...
            for (var i = Count - 1; i >= 0; i--) {
                ...
                if (!(curve.IsBar && pane._barSettings.Type == BarType.SortedOverlay)) curve.Draw(g, pane, pos, scaleFactor);
            }
        }
        ...
        public int GetBarItemPos(GraphPane pane, BarItem barItem) {
            if (pane._barSettings.Type == BarType.Overlay ||
                pane._barSettings.Type == BarType.Stack ||
                    pane._barSettings.Type == BarType.PercentStack) return 0;

            var i = 0;
            foreach (var curve in this) {
                if (curve == barItem) return i;
                if (curve is BarItem) i++;
            }
