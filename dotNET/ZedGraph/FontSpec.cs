...
namespace ZedGraph {
    ...
    [Serializable] public class FontSpec : ICloneable, ISerializable {
        #region Fields
        /// <summary>
        ...
        void Remake(float scaleFactor, float size, ref float scaledSize, ref Font font) {
            ...
            if (((((font != null && Math.Abs(newSize - oldSize) <= 0.1) && font.Name == Family) && font.Bold == _isBold) &&
                font.Italic == _isItalic) && font.Underline == _isUnderline) return;
        ...
        public void Draw(
            ...
            var sizeF = layoutArea.IsEmpty ? MeasureString(g, text, scaleFactor) : MeasureString(g, text, scaleFactor, layoutArea);
            ...
            var strFormat = new StringFormat {Alignment = _stringAlignment};
            ...
            if (_isDropShadow) {
                ...
                using (var brushD = new SolidBrush(_dropShadowColor)) g.DrawString(text, _font, brushD, rectD, strFormat);
            }

            // make a solid brush for rendering the font itself
            using (var brush = new SolidBrush(_fontColor)) // Draw the actual text.  Note that the coordinate system
                // is set up such that 0,0 is at the location where the
                // CenterTop of the text needs to be.
        ...
        public float GetHeight(float scaleFactor) {
            ...
            if (_isDropShadow) height += (float) (Math.Sin(_dropShadowAngle) * _dropShadowOffset * _font.Height);
            return height;
        }
        ...
        public float GetWidth(Graphics g, string text, float scaleFactor) {
            ...
            if (_isDropShadow) width += (float) (Math.Cos(_dropShadowAngle) * _dropShadowOffset * _font.Height);
            return width;
        }
        ...
        public SizeF BoundingBox(Graphics g, string text, float scaleFactor, SizeF layoutArea) {
            //Remake( scaleFactor, this.Size, ref this.scaledSize, ref this.font );
            var s = layoutArea.IsEmpty ? MeasureString(g, text, scaleFactor) : MeasureString(g, text, scaleFactor, layoutArea);

            var cs = (float) Math.Abs(Math.Cos(_angle * Math.PI / 180.0));
            var sn = (float) Math.Abs(Math.Sin(_angle * Math.PI / 180.0));

            var s2 = new SizeF(
                s.Width * cs + s.Height * sn,
        ...
        public SizeF BoundingBoxTenPower(Graphics g, string text, float scaleFactor) {
            ...
            var s2 = new SizeF(
                totSize.Width * cs + totSize.Height * sn,
        ...
        public bool PointInBox(
            ...
            // Get the width and height of the text
            var sizeF = layoutArea.IsEmpty ? g.MeasureString(text, _font) : g.MeasureString(text, _font, layoutArea);
        ...
        Matrix SetupMatrix(
            ...
            if (_angle != 0.0F) matrix.Rotate(-angle, MatrixOrder.Prepend);
            ...
            float xa, ya;
            switch (alignH) {
                case AlignH.Left: xa = sizeF.Width / 2.0F; break;
                case AlignH.Right: xa = -sizeF.Width / 2.0F; break;
                default: xa = 0.0F; break;
            }

            switch (alignV) {
                case AlignV.Center: ya = -sizeF.Height / 2.0F; break;
                case AlignV.Bottom: ya = -sizeF.Height; break;
                default: ya = 0.0F; break;
            }
        ...
        static Matrix GetMatrix(
            ...
            // Rotate the coordinate system according to the 
            // specified angle of the FontSpec
            if (angle != 0.0F) matrix.Rotate(angle, MatrixOrder.Prepend);
