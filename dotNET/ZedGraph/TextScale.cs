...
namespace ZedGraph {
    ...
    [Serializable] internal class TextScale : Scale //, ICloneable
    {
        #region constructors
        ...
        internal override int CalcNumTics() {
            ...
            var nTics = _textLabels == null ? 10 : _textLabels.Length;
        ...
        public override void PickScale(GraphPane pane, Graphics g, float scaleFactor) {
            ...
            if (_textLabels != null) {
                if (_minAuto) _min = 0.5;
                if (_maxAuto) _max = _textLabels.Length + 0.5;
            } else {
                if (_minAuto) _min -= 0.5;
                if (_maxAuto) _max += 0.5;
            }
            // Test for trivial condition of range = 0 and pick a suitable default
            if (_max - _min < .1)
                if (_maxAuto) _max = _min + 10.0;
                else _min = _max - 10.0;

            if (_majorStepAuto)
                if (!_isPreventLabelOverlap) _majorStep = 1;
                    ...
                    _majorStep = tmpStep;
                } else _majorStep = (int) ((_max - _min - 1.0) / Default.MaxTextLabels) + 1.0;
            else {
                _majorStep = (int) _majorStep;
                if (_majorStep <= 0) _majorStep = 1.0;
            }
        ...
        internal override string MakeLabel(GraphPane pane, int index, double dVal) {
            ...
            if (_textLabels == null || index < 0 || index >= _textLabels.Length) return string.Empty;
