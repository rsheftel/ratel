...
namespace ZedGraph {
    ...
    [Serializable] internal class ExponentScale : Scale //, ICloneable
    {
        #region constructors
        /// <summary>
        ...
        internal override double CalcMajorTicValue(double baseVal, double tic) {
            if (_exponent > 0.0) //return baseVal + Math.Pow ( (double) this.majorStep * tic, exp );
                //baseVal is got from CalBase..., and it is exp..
                return Math.Pow(Math.Pow(baseVal, 1 / _exponent) + _majorStep * tic, _exponent);
            return _exponent < 0.0 ? Math.Pow(Math.Pow(baseVal, 1 / _exponent) + _majorStep * tic, _exponent) : 1.0;
        }
        ...
        public override void PickScale(GraphPane pane, Graphics g, float scaleFactor) {
            ...
            if (_majorStepAuto) {
                ...
                if (_isPreventLabelOverlap) {
                    ...
                    if (maxLabels < (_max - _min) / _majorStep) _majorStep = CalcBoundedStepSize(this, _max - _min, maxLabels);
                }
            }

            // Calculate the new step size
            if (_minorStepAuto)
                _minorStep = CalcStepSize(
                    _majorStep,
            ...
            if (_minAuto) _min = _min - MyMod(_min, _majorStep);
            ...
            if (_magAuto) {
                ...
                if (Math.Abs(_min) > 1.0e-10) mag = Math.Floor(Math.Log10(Math.Abs(_min)));
                if (Math.Abs(_max) > 1.0e-10) mag2 = Math.Floor(Math.Log10(Math.Abs(_max)));
                if (Math.Abs(mag2) > Math.Abs(mag)) mag = mag2;
