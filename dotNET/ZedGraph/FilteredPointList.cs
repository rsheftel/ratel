...
namespace ZedGraph {
    ...
    [Serializable] public class FilteredPointList : IPointList {
        #region Fields
        /// <summary>
        ...
        public PointPair this[int index] {
            get {
                ...
                if (_minBoundIndex >= 0 && _maxBoundIndex >= 0 && _maxPts >= 0) {
                    ...
                    if (nPts > _maxPts) // if we're skipping points, then calculate the new index
                ...
                double xVal, yVal;
                if (index >= 0 && index < _x.Length) xVal = _x[index];
                else xVal = PointPairBase.Missing;

                if (index >= 0 && index < _y.Length) yVal = _y[index];
                else yVal = PointPairBase.Missing;
            ...
            set {
                ...
                if (index >= 0 && index < _x.Length) _x[index] = value.X;

                if (index >= 0 && index < _y.Length) _y[index] = value.Y;
            }
        }
        ...
        public int Count {
            get {
                ...
                if (_minBoundIndex >= 0 && _maxBoundIndex >= 0 && _maxPts > 0) {
                    ...
                    if (boundSize < arraySize) arraySize = boundSize;

                    // limit the point count to the declared max points
                    if (arraySize > _maxPts) arraySize = _maxPts;
                }
