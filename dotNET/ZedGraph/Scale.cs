...
namespace ZedGraph {
    ...
    [Serializable] public abstract class Scale : ISerializable {
        #region Fields
        ...
        protected Scale(Axis ownerAxis) {
            ...
            _fontSpec = new FontSpec(
                ...
                Default.FillColor,
                Default.FillBrush,
                Default.FillType) {Border = {IsVisible = false}};
        ...
        protected Scale(Scale rhs, Axis owner) {
            ...
            if (rhs._textLabels != null) _textLabels = (string[]) rhs._textLabels.Clone();
            else _textLabels = null;
        }
...
*/
        ...
        public double MajorStep {
            ...
            set {
                if (value < 1e-300) _majorStepAuto = true;
                else {
                    _majorStep = value;
        ...
        public double MinorStep {
            ...
            set {
                if (value < 1e-300) _minorStepAuto = true;
                else {
                    _minorStep = value;
        ...
        public FontSpec FontSpec {
            ...
            set {
                if (value == null) throw new ArgumentNullException("Uninitialized FontSpec in Scale");
                _fontSpec = value;
            }
        }
...
*/
        ...
        internal virtual string MakeLabel(GraphPane pane, int index, double dVal) {
            if (_format == null) _format = Default.Format;
        ...
        internal SizeF GetScaleMaxSpace(
            Graphics g,
            GraphPane pane,
            float scaleFactor,
            bool applyAngle) {
            ...
            if (!applyAngle) _fontSpec.Angle = 0;
            ...
            for (i = 0; i < nTics; i++) {
                ...
                if (IsLog && _isUseTenPower)
                    sizeF = _fontSpec.BoundingBoxTenPower(
                        g,
                        tmpStr,
                        scaleFactor);
                else
                    sizeF = _fontSpec.BoundingBox(
                        g,
                        tmpStr,
                        scaleFactor);

                if (sizeF.Height > maxSpace.Height) maxSpace.Height = sizeF.Height;
                if (sizeF.Width > maxSpace.Width) maxSpace.Width = sizeF.Width;
            }
        ...
        internal virtual double CalcBaseTic() {
            if (_baseTic != PointPairBase.Missing) return _baseTic;
            if (IsAnyOrdinal) // basetic is always 1 for ordinal types
                return 1;
...
//			MajorGrid grid = _ownerAxis._majorGrid;
            ...
            using (var ticPen = tic.GetPen(pane, scaleFactor)) //			using ( Pen gridPen = grid.GetPen( pane, scaleFactor ) )
            {
                ...
                for (var i = firstTic; i < nTics + firstTic; i++) {
                    ...
                    if (dVal < _minLinTemp) continue;
                    // if we've already past the end of the scale, then we're done
                    if (dVal > _maxLinTemp + rangeTol) break;
...
//					grid.Draw( g, gridPen, pixVal2, topPix );
                    ...
                    if (IsPreventLabelOverlap &&
                        Math.Abs(pixVal - lastPixVal) < maxLabelSize.Width) continue;
...
//				float lastPixVal = -10000;
        ...
        internal void DrawLabel(
            ...
            if (_ownerAxis.MajorTic.IsOutside) textTop = scaledTic + charHeight * _labelGap;
            else textTop = charHeight * _labelGap;
            ...
            float height;
            if (IsLog && _isUseTenPower) height = _fontSpec.BoundingBoxTenPower(g, tmpStr, scaleFactor).Height;
            else height = _fontSpec.BoundingBox(g, tmpStr, scaleFactor).Height;

            switch (_align) {
                case AlignP.Center: textCenter = textTop + maxSpace / 2.0F; break;
                case AlignP.Outside: textCenter = textTop + maxSpace - height / 2.0F; break;
                default: textCenter = textTop + height / 2.0F; break;
            }

            if (_isLabelsInside) textCenter = shift - textCenter;
            else textCenter = shift + textCenter;

            var av = AlignV.Center;
            var ah = AlignH.Center;

            if (_ownerAxis is XAxis || _ownerAxis is X2Axis) ah = _alignH;
            else av = _alignH == AlignH.Left ? AlignV.Top : (_alignH == AlignH.Right ? AlignV.Bottom : AlignV.Center);

            if (IsLog && _isUseTenPower)
                _fontSpec.DrawTenPower(
                    g,
                    pane,
                    tmpStr,
                    pixVal,
        ...
        internal void Draw(Graphics g, GraphPane pane, float scaleFactor, float shiftPos) {
            ...
            using (var pen = new Pen(
                ...
                if (_ownerAxis.IsAxisSegmentVisible) g.DrawLine(pen, 0.0F, shiftPos, rightPix, shiftPos);
...
//			_ownerAxis.DrawMinorTics( g, pane, baseVal, shiftPos, scaleFactor, topPix );
        ...
        internal void GetTopRightPix(GraphPane pane, out float topPix, out float rightPix) {
            ...
            if (IsLog) return;
            if (_majorStep <= 0 || _minorStep <= 0) return;
        ...
        public float GetClusterWidth(double clusterScaleWidth) {
            var basisVal = _min;
            return Math.Abs(
                Transform(basisVal + clusterScaleWidth) -
                    Transform(basisVal));
        }
        #endregion
        ...
        public virtual void PickScale(GraphPane pane, Graphics g, float scaleFactor) {
            ...
            if (Double.IsInfinity(minVal) || Double.IsNaN(minVal) || minVal == Double.MaxValue) minVal = 0.0;
            if (Double.IsInfinity(maxVal) || Double.IsNaN(maxVal) || maxVal == Double.MaxValue) maxVal = 0.0;
            ...
            if (_minAuto) {
                ...
                if (numType && (_min < 0 || minVal - _minGrace * range >= 0.0)) _min = minVal - _minGrace * range;
            }
            if (_maxAuto) {
                _max = maxVal;
                // Do not let the grace value extend the axis above zero when all the values were negative
                if (numType && (_max > 0 || maxVal + _maxGrace * range <= 0.0)) _max = maxVal + _maxGrace * range;
            }
            ...
            if (_max <= _min)
                if (_maxAuto) _max = _min + 1.0;
                else if (_minAuto) _min = _max - 1.0;
        }
        ...
        public int CalcMaxLabels(Graphics g, GraphPane pane, float scaleFactor) {
            ...
            if (costh > 0.001) maxWidth = size.Width / costh;
            if (sinth > 0.001) temp = size.Height / sinth;
			...
			*/
            ...
            if (_ownerAxis is XAxis || _ownerAxis is X2Axis) width = (chartRect.Width == 0) ? pane.Rect.Width * 0.75 : chartRect.Width;
            else width = (chartRect.Height == 0) ? pane.Rect.Height * 0.75 : chartRect.Height;

            var maxLabels = (int) (width / maxWidth);
            if (maxLabels <= 0) maxLabels = 1;
        ...
        internal void SetScaleMag(double min, double max, double step) {
            ...
            if (_magAuto) {
                ...
                if (Math.Abs(_min) > 1.0e-30) mag = Math.Floor(Math.Log10(Math.Abs(_min)));
                if (Math.Abs(_max) > 1.0e-30) mag2 = Math.Floor(Math.Log10(Math.Abs(_max)));

                mag = Math.Max(mag2, mag);

                // Do not use scale multiples for magnitudes below 4
                if (mag == -100 || Math.Abs(mag) <= 3) mag = 0;
        ...
        /// to divide the range into</param>
        /// <returns>The calculated step size for the specified data range.</returns>
        protected static double CalcBoundedStepSize(Scale scale, double range, double maxSteps) {
            // Calculate an initial guess at step size
        ...
        internal virtual int CalcNumTics() {
            ...
            var nTics = (int) ((_max - _min) / _majorStep + 0.01) + 1;
        ...
        internal void SetRange(GraphPane pane, Axis axis) {
            if (_rangeMin >= Double.MaxValue || _rangeMax <= Double.MinValue) // If this is a Y axis, and the main Y axis is valid, use it for defaults
        ...
        public float Transform(double x) {
            ...
            double ratio;
            if (denom > 1e-100) ratio = (Linearize(x) - _minLinTemp) / denom;
            else ratio = 0;
            ...
            if (_isReverse == (_ownerAxis is XAxis || _ownerAxis is X2Axis)) return (float) (_maxPix - (_maxPix - _minPix) * ratio);
        ...
        public float Transform(bool isOverrideOrdinal, int i, double x) {
            ...
            if (IsAnyOrdinal && i >= 0 && !isOverrideOrdinal) x = i + 1.0;
            return Transform(x);
        }
        ...
        public float LocalTransform(double x) {
            ...
            var ratio = (x - _minLinTemp) /
                (_maxLinTemp - _minLinTemp);

            if (_isReverse == (_ownerAxis is YAxis || _ownerAxis is X2Axis)) rv = (float) ((_maxPix - _minPix) * ratio);
            else rv = (float) ((_maxPix - _minPix) * (1.0F - ratio));
