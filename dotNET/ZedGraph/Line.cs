...
namespace ZedGraph {
    ...
    [Serializable] public class Line : LineBase, ICloneable {
        #region Fields
        /// <summary>
        ...
        protected Line(SerializationInfo info, StreamingContext context)
            ...
            if (sch >= 13) _isOptimizedDraw = info.GetBoolean("isOptimizedDraw");
        }
        ...
        public void Draw(Graphics g, GraphPane pane, CurveItem curve, float scaleFactor) {
            ...
            if (curve is StickItem) DrawSticks(g, pane, curve, scaleFactor);
            else if (IsSmooth || Fill.IsVisible) DrawSmoothFilledCurve(g, pane, curve, scaleFactor);
            else DrawCurve(g, pane, curve, scaleFactor);
        ...
        public void DrawSegment(
            ...
            float y2,
            float scaleFactor) {
            if (_isVisible && !Color.IsEmpty) using (var pen = GetPen(pane, scaleFactor)) g.DrawLine(pen, x1, y1, x2, y2);
        }
        ...
        public void DrawSticks(Graphics g, GraphPane pane, CurveItem curve, float scaleFactor) {
            var source = this;
            if (curve.IsSelected) source = Selection.Line;
            ...
            using (var pen = source.GetPen(pane, scaleFactor))
                for (var i = 0; i < curve.Points.Count; i++) {
                    ...
                    if (pt.X == PointPairBase.Missing || pt.Y == PointPairBase.Missing || Double.IsNaN(pt.X) ||
                        Double.IsNaN(pt.Y) || Double.IsInfinity(pt.X) || Double.IsInfinity(pt.Y) ||
                            (xAxis._scale.IsLog && pt.X <= 0.0) || (yAxis._scale.IsLog && pt.Y <= 0.0)) continue;
                    ...
                    if (pixX < pane.Chart._rect.Left || pixX > pane.Chart._rect.Right) continue;
                    if (pixY > pane.Chart._rect.Bottom) pixY = pane.Chart._rect.Bottom;
                    if (pixY < pane.Chart._rect.Top) pixY = pane.Chart._rect.Top;

                    if (!curve.IsSelected && _gradientFill.IsGradientValueType) using (var tPen = GetPen(pane, scaleFactor, pt)) g.DrawLine(tPen, pixX, pixY, pixX, basePix);
                    else g.DrawLine(pen, pixX, pixY, pixX, basePix);
                }
        }
        ...
        public void DrawSmoothFilledCurve(
            ...
            if (!IsVisible || Color.IsEmpty || points == null ||
                !BuildPointsArray(pane, curve, out arrPoints, out count) || count <= 2) return;
            ...
            if (_isSmooth)
                using (var pen = GetPen(pane, scaleFactor)) // Stroke the curve
        ...
        static bool IsFirstLine(GraphPane pane, CurveItem curve) {
            ...
            for (var j = 0; j < curveList.Count; j++) {
                ...
                if (tCurve is LineItem && tCurve.IsY2Axis == curve.IsY2Axis &&
                    tCurve.YAxisIndex == curve.YAxisIndex) return tCurve == curve;
            }
        ...
        public void DrawCurve(
            ...
            using (var pen = source.GetPen(pane, scaleFactor))
                if (points != null && !_color.IsEmpty && IsVisible) {
                    ...
                    if (isOptDraw) isPixelDrawn = new bool[maxX + 1,maxY + 1];
                    ...
                    for (var i = 0; i < points.Count; i++) {
                        ...
                        if (curX == PointPairBase.Missing ||
                            ...
                            var isOut = (tmpX < minX && lastX < minX) || (tmpX > maxX && lastX > maxX) ||
        ...
        void InterpolatePoint(
            ...
            } catch (Exception ex) { LogC.err("exception in InterpolatePoint", ex); throw;}
        }
        ...
        public bool BuildPointsArray(
            ...
            if (!IsVisible || Color.IsEmpty || points == null) return false;
            var index = 0;
            ...
            for (var i = 0; i < points.Count; i++) // make sure that the current point is valid
                if (!points[i].IsInvalid) {
                    ...
                    if (x == PointPairBase.Missing || y == PointPairBase.Missing) continue;

                    // Transform the user scale values to pixel locations
                    var xAxis = curve.GetXAxis(pane);
                    var curX = xAxis.Scale.Transform(curve.IsOverrideOrdinal, i, x);
                    var yAxis = curve.GetYAxis(pane);
                    var curY = yAxis.Scale.Transform(curve.IsOverrideOrdinal, i, y);

                    if (curX < -1000000 || curY < -1000000 || curX > 1000000 || curY > 1000000) continue;
        ...
        public bool BuildLowPointsArray(
            ...
            for (var i = points.Count - 1; i >= 0; i--) // Make sure the current point is valid
                if (!points[i].IsInvalid) {
                    // Get the user scale values for the current point
                    double x;
                    double y;
        ...
        public void CloseCurve(
            ...
            if (pane.LineType != LineType.Stack) {
                ...
                // the X axis crosses)
                var yAxis = curve.GetYAxis(pane);
                var yBase = yAxis.Scale.Transform(yMin);
                ...
                if (index > 0) {
                    for (var i = index - 1; i >= 0; i--) {
                        var tmpCurve = pane.CurveList[i];
                        if (!(tmpCurve is LineItem)) continue;
                        tension = ((LineItem) tmpCurve).Line.IsSmooth
                            ? ((LineItem) tmpCurve).Line.SmoothTension
                            : 0f;
                        break;
                    }
                }
