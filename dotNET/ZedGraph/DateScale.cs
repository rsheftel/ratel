...
namespace ZedGraph {
    ...
    [Serializable] internal class DateScale : Scale //, ICloneable
    {
        #region constructors
        /// <summary>
        ...
        public override void PickScale(GraphPane pane, Graphics g, float scaleFactor) {
            ...
            if (_majorStepAuto) {
                ...
                if (_isPreventLabelOverlap) {
                    ...
                    if (maxLabels < CalcNumTics()) _majorStep = CalcDateStepSize(_max - _min, maxLabels);
                }
            }
        ...
        internal static double CalcDateStepSize(double range, double targetSteps, Scale scale) {
            ...
            if (range > Default.RangeYearYear) {
                ...
                if (scale._formatAuto) scale._format = Default.FormatYearYear;

                tempStep = Math.Ceiling(tempStep / 365.0);

                if (scale._minorStepAuto) {
                    scale._minorUnit = DateUnit.Year;
                    scale._minorStep = tempStep == 1.0 ? 0.25 : CalcStepSize(tempStep, targetSteps);
                }
            } else if (range > Default.RangeYearMonth) {
                scale._majorUnit = DateUnit.Year;
                if (scale._formatAuto) scale._format = Default.FormatYearMonth;
                tempStep = Math.Ceiling(tempStep / 365.0);
                ...
                if (scale._minorStepAuto) {
                    ...
                    if (scale._minorStep > 6) scale._minorStep = 12;
                    else if (scale._minorStep > 3) scale._minorStep = 6;
                }
            } else if (range > Default.RangeMonthMonth) {
                scale._majorUnit = DateUnit.Month;
                if (scale._formatAuto) scale._format = Default.FormatMonthMonth;
                ...
                if (scale._formatAuto) scale._format = Default.FormatDayDay;
                ...
                scale._majorUnit = DateUnit.Day;
                if (scale._formatAuto) scale._format = Default.FormatDayHour;
                tempStep = Math.Ceiling(tempStep);
                ...
                if (scale._minorStepAuto) {
                    ...
                    // make sure the minorStep is 1, 2, 3, 6, or 12 hours
                    if (scale._minorStep > 6) scale._minorStep = 12;
                    else scale._minorStep = scale._minorStep > 3 ? 6 : 1;
                }
            } else if (range > Default.RangeHourHour) {
                scale._majorUnit = DateUnit.Hour;
                tempStep = Math.Ceiling(tempStep * XDate.HoursPerDay);
                if (scale._formatAuto) scale._format = Default.FormatHourHour;

                if (tempStep > 12.0) tempStep = 24.0;
                else if (tempStep > 6.0) tempStep = 12.0;
                else if (tempStep > 2.0) tempStep = 6.0;
                else tempStep = tempStep > 1.0 ? 2.0 : 1.0;

                if (scale._minorStepAuto) {
                    scale._minorUnit = DateUnit.Hour;
                    if (tempStep <= 1.0) scale._minorStep = 0.25;
                    else if (tempStep <= 6.0) scale._minorStep = 1.0;
                    else scale._minorStep = tempStep <= 12.0 ? 2.0 : 4.0;
                }
            } else if (range > Default.RangeHourMinute) {
                scale._majorUnit = DateUnit.Hour;
                tempStep = Math.Ceiling(tempStep * XDate.HoursPerDay);

                if (scale._formatAuto) scale._format = Default.FormatHourMinute;
                ...
                if (scale._minorStepAuto) {
                    ...
                    if (scale._minorStep > 15.0) scale._minorStep = 30.0;
                    else if (scale._minorStep > 5.0) scale._minorStep = 15.0;
                    else scale._minorStep = scale._minorStep > 1.0 ? 5.0 : 1.0;
                }
            } else if (range > Default.RangeMinuteMinute) {
                scale._majorUnit = DateUnit.Minute;
                if (scale._formatAuto) scale._format = Default.FormatMinuteMinute;
                ...
                else tempStep = tempStep > 1.0 ? 5.0 : 1.0;

                if (scale._minorStepAuto) {
                    scale._minorUnit = DateUnit.Minute;
                    if (tempStep <= 1.0) scale._minorStep = 0.25;
                    else scale._minorStep = tempStep <= 5.0 ? 1.0 : 5.0;
                }
            } else if (range > Default.RangeMinuteSecond) {
                scale._majorUnit = DateUnit.Minute;
                tempStep = Math.Ceiling(tempStep * XDate.MinutesPerDay);

                if (scale._formatAuto) scale._format = Default.FormatMinuteSecond;
            ...
            } else if (range > Default.RangeSecondSecond) // SecondSecond
            {
                ...
                if (scale._formatAuto) scale._format = Default.FormatSecondSecond;
            ...
            } else // MilliSecond
            {
                ...
                if (scale._formatAuto) scale._format = Default.FormatMillisecond;
            ...
            switch (_majorUnit) {
                default:
                    ...
                    if (direction == 1 && month == 1 && day == 1 && hour == 0
                        && minute == 0 && second == 0) return date;
                    return XDate.CalendarDateToXLDate(
                        year + direction,
                        1,
                        1,
                        0,
                ...
                case DateUnit.Day:
                    ...
                    if (direction == 1 && hour == 0 && minute == 0 && second == 0) return date;
                    return XDate.CalendarDateToXLDate(
                        year,
                        month,
                        day + direction,
                ...
                case DateUnit.Hour:
                    ...
                    if (direction == 1 && minute == 0 && second == 0) return date;
                    return XDate.CalendarDateToXLDate(
                        year,
                        month,
                        day,
                ...
                case DateUnit.Minute:
                    ...
                    if (direction == 1 && second == 0) return date;
                    return XDate.CalendarDateToXLDate(
                        year,
                        month,
                        day,
        ...
        /// </returns>
        static double GetUnitMultiple(DateUnit unit) {
            switch (unit) {
                default:
                    return 365.0;
                case DateUnit.Month:
