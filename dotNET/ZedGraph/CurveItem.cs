...
namespace ZedGraph {
    ...
    [Serializable] public abstract class CurveItem : ISerializable, ICloneable {
        #region Fields
        /// <summary>
        ...
        protected CurveItem(string label, double[] x, double[] y) :
...
*/
        ...
        protected CurveItem(string label, IPointList points) {
            Init(label);
            _points = points ?? new PointPairList();
        }
        ...
        /// </summary>
        /// <param name="label">A string label (legend entry) for this curve</param>
        protected CurveItem(string label) : this(label, null) {}
        ...
        protected CurveItem(CurveItem rhs) {
            ...
            if (rhs.Tag is ICloneable) Tag = ((ICloneable) rhs.Tag).Clone();
            else Tag = rhs.Tag;
        ...
        protected CurveItem(SerializationInfo info, StreamingContext context) {
            ...
            _isX2Axis = sch >= 11 && info.GetBoolean("isX2Axis");
        ...
        [SecurityPermission(SecurityAction.Demand, SerializationFormatter = true)] public virtual void GetObjectData(
            ...
            PointPairList list;
            if (_points is PointPairList) list = _points as PointPairList;
            else list = new PointPairList(_points);
        ...
        public Color Color {
            get {
                if (this is BarItem) return ((BarItem) this).Bar.Fill.Color;
                if (this is LineItem && ((LineItem) this).Line.IsVisible) return ((LineItem) this).Line.Color;
                if (this is LineItem) return ((LineItem) this).Symbol.Border.Color;
                if (this is ErrorBarItem) return ((ErrorBarItem) this).Bar.Color;
                if (this is HiLowBarItem) return ((HiLowBarItem) this).Bar.Fill.Color;
                return Color.Empty;
            }
            set {
                if (this is BarItem) ((BarItem) this).Bar.Fill.Color = value;
                ...
                } else if (this is ErrorBarItem) ((ErrorBarItem) this).Bar.Color = value;
                else if (this is HiLowBarItem) ((HiLowBarItem) this).Bar.Fill.Color = value;
            }
        }
        ...
        public PointPair this[int index] {
            get { return _points == null ? new PointPair(PointPairBase.Missing, PointPairBase.Missing) : (_points)[index]; }
        }
        ...
        public void AddPoint(PointPair point) {
            if (_points == null) Points = new PointPairList();

            if (_points is IPointListEdit) (_points as IPointListEdit).Add(point);
        ...
        /// </remarks>
        public void Clear() {
            if (_points is IPointListEdit) (_points as IPointListEdit).Clear();
            else throw new NotImplementedException();
        }
        ...
        public void RemovePoint(int index) {
            if (_points is IPointListEdit) (_points as IPointListEdit).RemoveAt(index);
            else throw new NotImplementedException();
        }
        ...
        public Axis GetXAxis(GraphPane pane) {
            if (_isX2Axis) return pane.X2Axis;
            return pane.XAxis;
        }
        ...
        public Axis GetYAxis(GraphPane pane) {
            if (_isY2Axis) return _yAxisIndex < pane.Y2AxisList.Count ? pane.Y2AxisList[_yAxisIndex] : pane.Y2AxisList[0];
            return _yAxisIndex < pane.YAxisList.Count ? pane.YAxisList[_yAxisIndex] : pane.YAxisList[0];
        }
        ...
        public int GetYAxisIndex(GraphPane pane) {
            if (_yAxisIndex >= 0 &&
                _yAxisIndex < (_isY2Axis ? pane.Y2AxisList.Count : pane.YAxisList.Count)) return _yAxisIndex;
            return 0;
        }
        ...
        public virtual void GetRange(
            ...
            for (var i = 0; i < Points.Count; i++) {
                ...
                if (ignoreInitial && curY != 0 &&
                    curY != PointPairBase.Missing) ignoreInitial = false;

                if (ignoreInitial || outOfBounds || curX == PointPairBase.Missing || curY == PointPairBase.Missing) continue;
                ...
                if (isZIncluded && isXIndependent && curZ != PointPairBase.Missing) {
                    if (curZ < yMin) yMin = curZ;
                    if (curZ > yMax) yMax = curZ;
                } else if (isZIncluded && curZ != PointPairBase.Missing) {
                    if (curZ < xMin) xMin = curZ;
                    if (curZ > xMax) xMax = curZ;
                }
            }
        }
        ...
        public virtual Axis BaseAxis(GraphPane pane) {
            ...
            if (this is BarItem || this is ErrorBarItem || this is HiLowBarItem) barBase = pane._barSettings.Base;
            else barBase = _isX2Axis ? BarBase.X2 : BarBase.X;
        ...
        public virtual Axis ValueAxis(GraphPane pane) {
            ...
            if (this is BarItem || this is ErrorBarItem || this is HiLowBarItem) barBase = pane._barSettings.Base;
            else barBase = BarBase.X;

            if (barBase == BarBase.X || barBase == BarBase.X2) return GetYAxis(pane);
...
//						pane.CalcScaleFactor() );
            else // BarItem or LineItem
            {
                ...
                var numBars = 1.0F;
                if (pane._barSettings.Type == BarType.Cluster) numBars = pane.CurveList.NumClusterableBars;
    ...
    /// </summary>
        ...
        public class Comparer : IComparer<CurveItem> {
            ...
            #region IComparer<CurveItem> Members
            /// <summary>
            /// Compares two <see cref="CurveItem"/>s using the previously specified index value
            /// and axis.  Sorts in descending order.
            /// </summary>
            ...
            public int Compare(CurveItem l, CurveItem r) {
                ...
                if (r.NPts <= index) r = null;
                if (l.NPts <= index) l = null;
                ...
                if (lVal == PointPairBase.Missing || Double.IsInfinity(lVal) || Double.IsNaN(lVal)) l = null;
                if (rVal == PointPairBase.Missing || Double.IsInfinity(rVal) || Double.IsNaN(rVal)) r = null;

                if ((l == null && r == null) || (Math.Abs(lVal - rVal) < 1e-10)) return 0;
                if (l == null) return -1;
                if (r == null) return 1;
                return rVal < lVal ? -1 : 1;
            }
