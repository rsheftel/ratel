...
namespace ZedGraph {
    ...
    [Serializable] public class GraphObjList : List<GraphObj>, ICloneable {
        #region Constructors
        /// <summary>
        ...
        public GraphObjList(IEnumerable<GraphObj> rhs) {
            foreach (var item in rhs) Add((GraphObj) ((ICloneable) item).Clone());
        }
...
*/
        ...
        public void Draw(
            ...
            for (var i = Count - 1; i >= 0; i--) {
                var item = this[i];
                if (item.ZOrder != zOrder || !item.IsVisible) continue;
                Region region = null;
                ...
                if (region != null && item.IsClippedToChartRect && pane is GraphPane) g.Clip = region;
            }
        }
        ...
        public bool FindPoint(PointF mousePt, PaneBase pane, Graphics g, float scaleFactor, out int index) {
            ...
            for (var i = Count - 1; i >= 0; i--) if (this[i].PointInBox(mousePt, pane, g, scaleFactor)) if ((index >= 0 && this[i].ZOrder > this[index].ZOrder) || index < 0) index = i;
