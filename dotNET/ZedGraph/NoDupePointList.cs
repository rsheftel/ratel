...
namespace ZedGraph {
    ...
    [Serializable] public class NoDupePointList : List<DataPoint>, IPointListEdit {
        /// <summary>
        ...
        public NoDupePointList(NoDupePointList rhs) {
            ...
            for (var i = 0; i < count; i++) Add(rhs.GetDataPointAt(i));

            _filteredCount = rhs._filteredCount;
            _isFiltered = rhs._isFiltered;
            _filterMode = rhs._filterMode;

            if (rhs._visibleIndicies != null) _visibleIndicies = (int[]) rhs._visibleIndicies.Clone();
            else _visibleIndicies = null;
        }
        ...
        public new PointPair this[int index] {
            get {
                var j = index;
                if (_isFiltered) j = _visibleIndicies[index];
        ...
        public new int Count {
            get {
                return !_isFiltered ? base.Count : _filteredCount;
            }
        }
        ...
        public void Add(PointPair pt) {
            var dp = new DataPoint {X = pt.X, Y = pt.Y};
            Add(dp);
        }
        ...
        public void FilterData(GraphPane pane, Axis xAxis, Axis yAxis) {
            if (_visibleIndicies == null || _visibleIndicies.Length < base.Count) _visibleIndicies = new int[base.Count];
            ...
            if (width <= 0 || height <= 0) throw new IndexOutOfRangeException("Error in FilterData: Chart rect not valid");

            var usedArray = new bool[width,height];
            for (var i = 0; i < width; i++) for (var j = 0; j < height; j++) usedArray[i, j] = false;

            xAxis.Scale.SetupScaleData(pane, xAxis);
            yAxis.Scale.SetupScaleData(pane, yAxis);

            var n = _filterMode < 0 ? 0 : _filterMode;
            var left = (int) pane.Chart.Rect.Left;
            var top = (int) pane.Chart.Rect.Top;

            for (var i = 0; i < base.Count; i++) {
                var dp = base[i];
                var x = (int) (xAxis.Scale.Transform(dp.X) + 0.5) - left;
                var y = (int) (yAxis.Scale.Transform(dp.Y) + 0.5) - top;

                if (x < 0 || x >= width || y < 0 || y >= height) continue;
                var used = false;
                if (n <= 0) used = usedArray[x, y];
                else for (var ix = x - n; ix <= x + n; ix++) for (var iy = y - n; iy <= y + n; iy++) used |= (ix >= 0 && ix < width && iy >= 0 && iy < height && usedArray[ix, iy]);
