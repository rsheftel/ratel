...
namespace ZedGraph {
    ...
    [Serializable] public class Symbol : ICloneable, ISerializable {
        #region Fields
        /// <summary>
        ...
        public Symbol(Symbol rhs) {
            ...
            if (rhs.UserSymbol != null) _userSymbol = rhs.UserSymbol.Clone() as GraphicsPath;
            else _userSymbol = null;
        }
        ...
        protected Symbol(SerializationInfo info, StreamingContext context) {
            ...
            if (sch >= 11) _userSymbol = (GraphicsPath) info.GetValue("userSymbol", typeof (GraphicsPath));
            else _userSymbol = null;
        }
        ...
        void DrawSymbol(
            ...
            if (!_isVisible || Type == SymbolType.None || x >= 100000 || x <= -100000 || y >= 100000 || y <= -100000) return;
            var saveMatrix = g.Transform;
            g.TranslateTransform(x, y);

            // Fill or draw the symbol as required
            if (_fill.IsVisible) g.FillPath(brush, path);
            //FillPoint( g, x, y, scaleFactor, pen, brush );

            if (_border.IsVisible) g.DrawPath(pen, path);
        ...
        public void DrawSymbol(
            ...
            using (var pen = _border.GetPen(pane, scaleFactor, dataValue)) using (var path = MakePath(g, scaleFactor)) using (var brush = Fill.MakeBrush(path.GetBounds(), dataValue)) DrawSymbol(g, x, y, path, pen, brush);
        ...
        public GraphicsPath MakePath(Graphics g, float scaleFactor) {
            ...
            switch (
                _type == SymbolType.Default || (_type == SymbolType.UserDefined && _userSymbol == null)
                    ? Default.Type
                    : _type) {
                        ...
                        case SymbolType.UserDefined:
                            if (_userSymbol != null) path = _userSymbol.Clone() as GraphicsPath;
                            var scaleTransform = new Matrix(scaledSize, 0.0f, 0.0f, scaledSize, 0.0f, 0.0f);
                            if (path != null) path.Transform(scaleTransform);
                            break;
            }
