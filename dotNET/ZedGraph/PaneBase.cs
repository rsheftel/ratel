...
namespace ZedGraph {
    ...
    public abstract class PaneBase : ICloneable {
        ...
        protected PaneBase(PaneBase rhs) {
            ...
            if (rhs._tag is ICloneable) _tag = ((ICloneable) rhs._tag).Clone();
            else _tag = rhs._tag;
        }
        ...
        /// </param>
        protected PaneBase(SerializationInfo info) {
        ...
        public float CalcScaleFactor() {
            ...
            if (_rect.Height <= 0) return 1.0F;
            var length = _rect.Width;
            var aspect = _rect.Width / _rect.Height;
            if (aspect > ASPECTLIMIT) length = _rect.Height * ASPECTLIMIT;
            if (aspect < 1.0F / ASPECTLIMIT) length = _rect.Width * ASPECTLIMIT;

            var scaleFactor = length / (_baseDimension * 72F);
        ...
        public Bitmap GetImage(int width, int height, float dpi, bool isAntiAlias) {
            ...
            using (var bitmapGraphics = Graphics.FromImage(bitmap)) MakeImage(bitmapGraphics, width, height, isAntiAlias);
        ...
        internal static void SetAntiAliasMode(Graphics g, bool isAntiAlias) {
        ...
        void MakeImage(Graphics g, int width, int height, bool antiAlias) {
            //g.SmoothingMode = SmoothingMode.AntiAlias;
		...
		*/
        ...
        internal PointF TransformCoord(double x, double y, CoordType coord) {
            ...
            if (this is GraphPane) {
                ...
                if (gPane != null) chartRect = gPane.Chart._rect;
            }
            ...
            switch (coord) {
                ...
                case CoordType.XScaleYChartFraction:
                    if (gPane != null) ptPix.X = gPane.XAxis.Scale.Transform(x);
                    ptPix.Y = (float) (chartRect.Top + y * chartRect.Height);
                    break;
                case CoordType.XChartFractionYScale:
                    ptPix.X = (float) (chartRect.Left + x * chartRect.Width);
                    if (gPane != null) ptPix.Y = gPane.YAxis.Scale.Transform(y);
                    break;
                case CoordType.XChartFractionY2Scale:
                    ptPix.X = (float) (chartRect.Left + x * chartRect.Width);
                    if (gPane != null) ptPix.Y = gPane.Y2Axis.Scale.Transform(y);
                    break;
