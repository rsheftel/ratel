...
namespace ZedGraph {
    ...
    [Serializable] public class PieItem : CurveItem, ICloneable {
        #region Fields
        /*
		/// <summary>
...
*/
        static readonly ColorSymbolRotator _rotator = new ColorSymbolRotator();
        /// <summary>
        ...
        public PieLabelType LabelType {
            ...
            set {
                _labelType = value;
                LabelDetail.IsVisible = value != PieLabelType.None;
            }
        }
 ...
 */
        ...
        public PieItem(
            ...
            if (!color1.IsEmpty && !color2.IsEmpty) _fill = new Fill(color1, color2, fillAngle);
        }
        ...
        public override void Draw(Graphics g, GraphPane pane, int pos, float scaleFactor) {
            if (pane.Chart._rect.Width <= 0 && pane.Chart._rect.Height <= 0) //pane.PieRect = RectangleF.Empty;
                SlicePath = null;
            else {
                ...
                if (tRect.Width >= 1 && tRect.Height >= 1) {
                    ...
                    using (var brush = tFill.MakeBrush(_boundingRectangle)) {
                        ...
                        if (_labelType != PieLabelType.None) DrawLabel(g, pane, tRect, scaleFactor);
        ...
        void CalcExplodedRect(ref RectangleF explRect) {
        ...
        static void CalculatePieChartParams(GraphPane pane, ref double maxDisplacement) {
            ...
            foreach (PieItem curve in pane.CurveList)
                if (curve.IsPie) {
                    ...
                    if (curve.Displacement > maxDisplacement) maxDisplacement = curve.Displacement;
                }
        ...
        public void DrawLabel(Graphics g, GraphPane pane, RectangleF rect, float scaleFactor) {
            if (!_labelDetail.IsVisible) return;
        ...
        public void DesignLabel(Graphics g, GraphPane pane, RectangleF rect, float scaleFactor) {
            ...
            if (_midAngle > 315 || _midAngle <= 45) {
                ...
                if (size.Width > fill) //need to wrap, so create label rectangle for overloaded DrawString - two rows, max
            ...
            if (_midAngle > 45 && _midAngle <= 135) {
                ...
                if (size.Height / 2 > fill) //no, so got to move explosion radius
            ...
            if (_midAngle > 135 && _midAngle <= 225) {
                ...
                if (size.Width > fill) _labelDetail.LayoutArea = new SizeF(fill, size.Height * 3.0F);
            }
        ...
        void CalculateLinePoints(RectangleF rect, double midAngle) {
        ...
        static void BuildLabelString(PieItem curve) {
        ...
        static void CalcNewBaseRect(double maxDisplacement, ref RectangleF baseRect) {
        ...
        public override void DrawLegendKey(Graphics g, GraphPane pane, RectangleF rect, float scaleFactor) {
            ...
            // Fill the slice
            if (_fill.IsVisible) // just avoid height/width being less than 0.1 so GDI+ doesn't cry
                using (var brush = _fill.MakeBrush(rect)) g.FillRectangle(brush, rect);
                    //brush.Dispose();

            // Border the bar
            if (!_border.Color.IsEmpty) _border.Draw(g, pane, scaleFactor, rect);
        }
        ...
        public override bool GetCoords(GraphPane pane, int i, out string coords) {
            ...
            coords = String.Format(
                ...
                pts[1].X,
                pts[1].Y);
            for (var j = 2; j < count + 2; j++) coords += String.Format(j > count ? "{0:f0},{1:f0}" : "{0:f0},{1:f0},", pts[j].X, pts[j].Y);
