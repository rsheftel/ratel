...
namespace ZedGraph {
    ...
    [Serializable] public class LineBase : ICloneable, ISerializable {
        #region Fields
        /// <summary>
        ...
        public LineBase(Color color) {
            ...
            _gradientFill = new Fill(Color.Red, Color.White) {Type = FillType.None};
        }
        ...
        public Pen GetPen(PaneBase pane, float scaleFactor, PointPair dataValue) {
            var color = _color;
            if (_gradientFill.IsGradientValueType) color = _gradientFill.GetGradientColor(dataValue);

            var pen = new Pen(
                color,
                pane.ScaledPenWidth(_width, scaleFactor)) {DashStyle = _style};
