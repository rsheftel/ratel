...
namespace ZedGraph {
    ...
    [Serializable] public class PolyObj : BoxObj, ICloneable {
        #region Fields
        /// <summary>
        ...
        public override void Draw(Graphics g, PaneBase pane, float scaleFactor) {
            if (_points != null && _points.Length > 1)
                using (var path = MakePath(pane)) {
                    ...
                    if (_fill.IsVisible) using (var brush = Fill.MakeBrush(path.GetBounds())) g.FillPath(brush, path);

                    if (_border.IsVisible) using (var pen = _border.GetPen(pane, scaleFactor)) g.DrawPath(pen, path);
                }
        }
        ...
        internal GraphicsPath MakePath(PaneBase pane) {
            ...
            foreach (var pt in _points) {
                ...
                if (Math.Abs(pixPt.X) >= 100000 || Math.Abs(pixPt.Y) >= 100000) continue;
                if (first) first = false;
                else path.AddLine(lastPt, pixPt);

                lastPt = pixPt;
            }

            if (_isClosedFigure) path.CloseFigure();
        ...
        public override bool PointInBox(PointF pt, PaneBase pane, Graphics g, float scaleFactor) {
            if (_points == null || _points.Length <= 1) return false;
            if (! base.PointInBox(pt, pane, g, scaleFactor)) return false;

            using (var path = MakePath(pane)) return path.IsVisible(pt);
        }
        #endregion
    }
}