...
namespace ZedGraph {
    ...
    [Serializable] public class BarItem : CurveItem, ICloneable {
        #region Fields
        /// <summary>
        ...
        public static void CreateBarLabels(
            ...
            foreach (var curve in pane.CurveList) {
                ...
                for (var i = 0; i < points.Count; i++) {
                    ...
                    var centerVal = (float) valueHandler.BarCenterValue(
                        bar,
                        bar.GetBarWidth(pane),
                        i,
                        baseVal,
                        curveIndex);
                    ...
                    float position;
                    if (isBarCenter) position = (float) (hiVal + lowVal) / 2.0f;
                    else if (hiVal >= 0) position = (float) hiVal + labelOffset;
                    else position = (float) hiVal - labelOffset;
        ...
        public override bool GetCoords(GraphPane pane, int i, out string coords) {
            ...
            if (!_points[i].IsInvalid3D) {
                ...
                var pixLowVal = valueAxis.Scale.Transform(_isOverrideOrdinal, i, curLowVal);
                var pixHiVal = valueAxis.Scale.Transform(_isOverrideOrdinal, i, curHiVal);
                // calculate a pixel value for the center of the bar on the base axis
                var pixBase = baseAxis.Scale.Transform(_isOverrideOrdinal, i, curBase);
