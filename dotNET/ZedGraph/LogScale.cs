...
namespace ZedGraph {
    ...
    [Serializable] internal class LogScale : Scale //, ICloneable
    {
        #region constructors
        /// <summary>
        ...
        internal override double CalcBaseTic() {
            return _baseTic != PointPairBase.Missing ? _baseTic : Math.Ceiling(SafeLog(_min) - 0.00000001);
        }
        ...
        internal override int CalcNumTics() {
            ...
            var nTics = (int) ((SafeLog(_max) - SafeLog(_min)) / CyclesPerStep) + 1;
        ...
        public override void PickScale(GraphPane pane, Graphics g, float scaleFactor) {
            ...
            if (_majorStepAuto) _majorStep = 1.0;
            ...
            if (_min <= 0.0 && _max <= 0.0) {
                _min = 1.0;
                _max = 10.0;
            } else if (_min <= 0.0) _min = _max / 10.0;
            else if (_max <= 0.0) _max = _min * 10.0;

            // Test for trivial condition of range = 0 and pick a suitable default
            if (_max - _min < 1.0e-20) {
                if (_maxAuto) _max = _max * 2.0;
                if (_minAuto) _min = _min / 2.0;
            }
        ...
        internal override string MakeLabel(GraphPane pane, int index, double dVal) {
            ...
            return _isUseTenPower ? string.Format("{0:F0}", dVal) : Math.Pow(10.0, dVal).ToString(_format);
        }
        #endregion
