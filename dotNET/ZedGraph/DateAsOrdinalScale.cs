...
namespace ZedGraph {
    ...
    [Serializable] internal class DateAsOrdinalScale : Scale //, ICloneable
    {
        #region constructors
        /// <summary>
...
*/
                            ...
                            !Double.IsNaN(val2) &&
                                !Double.IsInfinity(val1) &&
                                    !Double.IsInfinity(val2) &&
                                        Math.Abs(val2 - val1) > 1e-10) range = Math.Abs(val2 - val1);
            }

            if (range > Default.RangeYearYear) _format = Default.FormatYearYear;
            else if (range > Default.RangeYearMonth) _format = Default.FormatYearMonth;
            else if (range > Default.RangeMonthMonth) _format = Default.FormatMonthMonth;
            else if (range > Default.RangeDayDay) _format = Default.FormatDayDay;
            else if (range > Default.RangeDayHour) _format = Default.FormatDayHour;
            else if (range > Default.RangeHourHour) _format = Default.FormatHourHour;
            else if (range > Default.RangeHourMinute) _format = Default.FormatHourMinute;
            else if (range > Default.RangeMinuteMinute) _format = Default.FormatMinuteMinute;
            else if (range > Default.RangeMinuteSecond) _format = Default.FormatMinuteSecond;
            else _format = range > Default.RangeSecondSecond ? Default.FormatSecondSecond : Default.FormatMillisecond;
        }
        ...
        internal override string MakeLabel(GraphPane pane, int index, double dVal) {
            ...
            if (pane.CurveList.Count <= 0 || pane.CurveList[0].Points.Count <= tmpIndex) return string.Empty;
            double val;
            if (_ownerAxis is XAxis || _ownerAxis is X2Axis) val = pane.CurveList[0].Points[tmpIndex].X;
            else val = pane.CurveList[0].Points[tmpIndex].Y;
