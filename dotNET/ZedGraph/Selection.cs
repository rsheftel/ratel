...
namespace ZedGraph {
    ...
    public class Selection : CurveList {
        ...
        public void AddToSelection(MasterPane master, CurveList ciList) {
            foreach (var ci in ciList) if (Contains(ci) == false) Add(ci);
    ...
    // Define a "Contains" method so that this class works with .Net 1.1 or 2.0
        ...
        public void ClearSelection(MasterPane master, bool sendEvent) {
            ...
            foreach (var pane in master.PaneList) foreach (var ci in pane.CurveList) ci.IsSelected = false;

            if (sendEvent) if (SelectionChangedEvent != null) SelectionChangedEvent(this, new EventArgs());
        }
        ...
        public void UpdateSelection(MasterPane master) {
            ...
            foreach (var pane in master.PaneList)
                foreach (var ci in pane.CurveList) //Make it Inactive
                    ci.IsSelected = false;
            foreach (var ci in  this) {
                ...
                if (ci.IsLine) //I don't know how to get a Pane, from a CurveItem, so I can only do it 
