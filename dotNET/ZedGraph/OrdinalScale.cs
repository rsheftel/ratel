...
namespace ZedGraph {
    ...
    [Serializable] internal class OrdinalScale : Scale //, ICloneable
    {
        #region constructors
        /// <summary>
        ...
        internal static void PickScale(GraphPane pane, Graphics g, float scaleFactor, Scale scale) {
            ...
            if (scale._max - scale._min < 1.0)
                if (scale._maxAuto) scale._max = scale._min + 0.5;
                else scale._min = scale._max - 0.5;
            else {
                ...
                if (scale._majorStepAuto) {
                    ...
                    if (scale.IsPreventLabelOverlap) {
                        ...
                        if (tmpStep > scale._majorStep) scale._majorStep = tmpStep;
                    }
                }

                scale._majorStep = (int) scale._majorStep;
                if (scale._majorStep < 1.0) scale._majorStep = 1.0;
                ...
                if (scale._minAuto) scale._min -= 0.5;
                if (scale._maxAuto) scale._max += 0.5;
            }
        }
        #endregion
