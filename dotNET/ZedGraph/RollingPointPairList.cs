...
namespace ZedGraph {
    ...
    /// </summary>
    [Serializable] public class RollingPointPairList : ISerializable, IPointListEdit {
        #region Fields
        /// <summary>
        ...
        public RollingPointPairList(int capacity, bool preLoad) {
            ...
            if (preLoad) for (var i = 0; i < capacity; i++) _mBuffer[i] = new PointPair();
        }
        ...
        public RollingPointPairList(IPointList rhs) {
            ...
            for (var i = 0; i < rhs.Count; i++) _mBuffer[i] = new PointPair(rhs[i]);
        ...
        public int Count {
            get {
                if (_headIdx == -1) return 0;

                if (_headIdx > _tailIdx) return (_headIdx - _tailIdx) + 1;

                if (_tailIdx > _headIdx) return (_mBuffer.Length - _tailIdx) + _headIdx + 1;
        ...
        public PointPair this[int index] {
            get {
                ...
                index += _tailIdx;
                if (index >= _mBuffer.Length) index -= _mBuffer.Length;
        ...
        int GetNextIndex() {
            if (_headIdx == -1) // buffer is currently empty.
                _headIdx = _tailIdx = 0;
            else {
                // Determine the index to write to.
                if (++_headIdx == _mBuffer.Length) // Wrap around.
                    _headIdx = 0;

                if (_headIdx == _tailIdx) // Buffer overflow. Increment tailIdx.
                    if (++_tailIdx == _mBuffer.Length) // Wrap around.
                        _tailIdx = 0;
            }
        ...
        public void Add(IPointList pointList) {
            ...
            for (var i = 0; i < pointList.Count; i++) Add(pointList[i]);
        }
        ...
        public PointPair Remove() {
            if (_tailIdx == -1) // buffer is currently empty.
        ...
        public void RemoveRange(int index, int count) {
            ...
            if (index >= totalCount || index < 0 || count < 0 || count > totalCount) throw new ArgumentOutOfRangeException();
        ...
        public void Add(double x, double y, double z, object tag) {
            ...
            if (_mBuffer[_headIdx] == null) _mBuffer[_headIdx] = new PointPair(x, y, z, tag);
            else {
