...
namespace ZedGraph {
    partial class ZedGraphControl {
        ...
        void contextMenuStrip1_Opening(object sender, CancelEventArgs e) {
            ...
            if (!_isShowContextMenu) return;
            var menuStr = string.Empty;

            var item = new ToolStripMenuItem {Name = "copy", Tag = "copy", Text = _resourceManager.GetString("copy")};
            ...
            item = new ToolStripMenuItem {Name = "print", Tag = "print", Text = _resourceManager.GetString("print")};
            ...
            item = new ToolStripMenuItem {Name = "unzoom", Tag = "unzoom"};

            if (pane == null || pane.ZoomStack.IsEmpty) menuStr = _resourceManager.GetString("unzoom");
            else
            ...
            if (pane == null || pane.ZoomStack.IsEmpty) item.Enabled = false;
            menuStrip.Items.Add(item);

            item = new ToolStripMenuItem {Name = "undo_all", Tag = "undo_all"};
            ...
            item = new ToolStripMenuItem {Name = "set_default", Tag = "set_default"};
            menuStr = _resourceManager.GetString("set_default");
            item.Text = menuStr;
            item.Click += MenuClick_RestoreScale;
            if (pane == null) item.Enabled = false;
            menuStrip.Items.Add(item);
            ...
            if (ContextMenuBuilder != null) ContextMenuBuilder(this, menuStrip, _menuClickPt, objState);
        }
        ...
        public void Copy(bool isShowMessage) {
            ...
            var str = _resourceManager.GetString("copied_to_clip");
        ...
        public void CopyEmf(bool isShowMessage) {
            ...
            // Threaded copy mode to avoid crash with MTA
            // Contributed by Dave Moor
            var ct = new Thread(ClipboardCopyThreadEmf);
            //ct.ApartmentState = ApartmentState.STA;
            ct.SetApartmentState(ApartmentState.STA);
            ct.Start();
            ct.Join();
        ...
        public String SaveAs(String DefaultFileName) {
            if (_masterPane != null) {
                ...
                if (!string.IsNullOrEmpty(DefaultFileName)) {
                    var ext = Path.GetExtension(DefaultFileName).ToLower();
                    switch (ext) {
                        case ".emf":
                    ...
                    if (DefaultFileName.Length > ext.Length) _saveFileDialog.FileName = DefaultFileName;
                }

                if (_saveFileDialog.ShowDialog() == DialogResult.OK) {
                    var myStream = _saveFileDialog.OpenFile();
        ...
        internal void SaveEmfFile(string fileName) {
            using (var g = CreateGraphics()) {
                var hdc = g.GetHdc();
                var metaFile = new Metafile(hdc, EmfType.EmfPlusOnly);
                using (var gMeta = Graphics.FromImage(metaFile)) //PaneBase.SetAntiAliasMode( gMeta, IsAntiAlias );
        ...
        protected void MenuClick_ShowValues(object sender, EventArgs e) {
            ...
            if (item != null) IsShowPointValues = !item.Checked;
        }
        ...
        public void RestoreScale(GraphPane primaryPane) {
            ...
            var oldState = new ZoomState(primaryPane, ZoomState.StateType.Zoom);
            ...
            using (var g = CreateGraphics()) {
                ...
                if (ZoomEvent != null) ZoomEvent(this, oldState, new ZoomState(primaryPane, ZoomState.StateType.Zoom));
        ...
        static void ResetAutoScale(GraphPane pane, Graphics g) {
            pane.XAxis.ResetAutoScale(pane, g);
            pane.X2Axis.ResetAutoScale(pane, g);
            foreach (var axis in pane.YAxisList) axis.ResetAutoScale(pane, g);
            foreach (var axis in pane.Y2AxisList) axis.ResetAutoScale(pane, g);
        }
		...
		*/
        ...
        public void ZoomOut(GraphPane primaryPane) {
            ...
            if (_isSynchronizeXAxes || _isSynchronizeYAxes)
                foreach (var pane in _masterPane._paneList) {
                    ...
                    if (pane == primaryPane) newState = state;
                }
            else newState = primaryPane.ZoomStack.Pop(primaryPane);

            // Provide Callback to notify the user of zoom events
            if (ZoomEvent != null) ZoomEvent(this, oldState, newState);
        ...
        public void ZoomOutAll(GraphPane primaryPane) {
            ...
            if (_isSynchronizeXAxes || _isSynchronizeYAxes)
                foreach (var pane in _masterPane._paneList) {
                    var state = pane.ZoomStack.PopAll(pane);
                    if (pane == primaryPane) newState = state;
                }
            else newState = primaryPane.ZoomStack.PopAll(primaryPane);
        ...
        internal class ClipboardMetafileHelper {
            ...
            [DllImport("gdi32.dll")] static extern IntPtr CopyEnhMetaFile(IntPtr hemfSrc, StringBuilder hNULL);
            ...
            internal static bool SaveEnhMetafileToFile(Metafile mf) {
                var hEMF = mf.GetHenhmetafile();
                if (!hEMF.Equals(new IntPtr(0))) {
                    var sfd = new SaveFileDialog {Filter = "Extended Metafile (*.emf)|*.emf", DefaultExt = ".emf"};
                    if (sfd.ShowDialog() == DialogResult.OK) {
                        var temp = new StringBuilder(sfd.FileName);
