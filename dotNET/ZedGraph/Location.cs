...
namespace ZedGraph {
    ...
    [Serializable] public class Location : ICloneable, ISerializable {
        ...
        public PointF TransformTopLeft(PaneBase pane, float width, float height) {
            var pt = Transform(pane);

            switch (_alignH) {
                case AlignH.Right: pt.X -= width; break;
                case AlignH.Center: pt.X -= width / 2.0F; break;
            }

            switch (_alignV) {
                case AlignV.Bottom: pt.Y -= height; break;
                case AlignV.Center: pt.Y -= height / 2.0F; break;
            }
        ...
        public RectangleF TransformRect(PaneBase pane) {
            var pix1 = TransformTopLeft(pane);
            var pix2 = TransformBottomRight(pane);
