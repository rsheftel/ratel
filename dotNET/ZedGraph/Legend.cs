...
namespace ZedGraph {
    ...
    [Serializable] public class Legend : ICloneable, ISerializable {
        ...
        public FontSpec FontSpec {
            ...
            set {
                if (value == null) throw new ArgumentNullException("Uninitialized FontSpec in Legend");
                _fontSpec = value;
            }
        }
        /// <summary>
        ...
        public Legend() {
            ...
            _fontSpec = new FontSpec(
                ...
                Default.FontFillType) {Border = {IsVisible = false}};
        ...
        public void Draw(Graphics g, PaneBase pane, float scaleFactor) {
            ...
            if (_hStack <= 0) _hStack = 1;
            if (_legendItemWidth <= 0) _legendItemWidth = 100;
            if (_legendItemHeight <= 0) _legendItemHeight = _tmpSize;
            ...
            using (new SolidBrush(Color.Black)) {
                foreach (var tmpPane in paneList) {
                    ...
                    for (var i = 0; i < count; i++) {
                        ...
                        if (curve._label._text == "" || !curve._label._isVisible) continue;
                        ...
                        if (_isShowLegendSymbols) {
                            ...
                            if (curve._label._fontSpec == null) tmpFont.FontColor = curve.Color;
                    ...
                    if (pane is MasterPane && ((MasterPane) pane).IsUniformLegendEntries) break;
                }

                // Draw a border around the legend if required
                if (iEntry > 0) Border.Draw(g, pane, scaleFactor, Rect);
            }
        }
            ...
            foreach (var tmpPane in paneList)
                foreach (var curve in tmpPane.CurveList)
                    if (curve._label._text != string.Empty && curve._label._isVisible) {
                        ...
                        if (curve._label._fontSpec != null) tmpHeight = curve._label._fontSpec.GetHeight(scaleFactor);

                        // Account for multiline legend entries
                        tmpHeight *= curve._label._text.Split('\n').Length;

                        if (tmpHeight > maxCharHeight) maxCharHeight = tmpHeight;
                    }
        ...
        public bool FindPoint(PointF mousePt, PaneBase pane, float scaleFactor, out int index) {
            ...
            if (!Rect.Contains(mousePt)) return false;
            var j = (int) ((mousePt.Y - Rect.Top) / _legendItemHeight);
            var i = (int) ((mousePt.X - Rect.Left - _tmpSize / 2.0f) / _legendItemWidth);
            if (i < 0) i = 0;
            if (i >= _hStack) i = _hStack - 1;
        ...
        static PaneList GetPaneList(PaneBase pane) {
            ...
            if (pane is GraphPane) {
                paneList = new PaneList {(GraphPane) pane};
            } else paneList = ((MasterPane) pane).PaneList;
        ...
        public void CalcRect(
            ...
            foreach (var tmpPane in paneList) {
                ...
                for (var i = 0; i < count; i++) {
                    var curve = tmpPane.CurveList[_isReverse ? count - i - 1 : i];
                    if (curve._label._text == string.Empty || !curve._label._isVisible) continue;
                    ...
                    if (curve is LineItem && ((LineItem) curve).Symbol.Size > _legendItemHeight) _legendItemHeight = ((LineItem) curve).Symbol.Size;
            ...
            if (_isHStack) {
                ...
                if (_isShowLegendSymbols) _legendItemWidth = 3.0f * _tmpSize + maxWidth;
                else _legendItemWidth = 0.5f * _tmpSize + maxWidth;
                ...
                if (maxWidth > 0) _hStack = (int) ((widthAvail - halfGap) / _legendItemWidth);

                // You can never have more columns than legend entries
                if (_hStack > nCurve) _hStack = nCurve;

                // a saftey check
                if (_hStack == 0) _hStack = 1;
            } else if (_isShowLegendSymbols) _legendItemWidth = 3.0F * _tmpSize + maxWidth;
            else _legendItemWidth = 0.5F * _tmpSize + maxWidth;
            ...
            if (_tmpSize > _legendItemHeight) _legendItemHeight = _tmpSize;
