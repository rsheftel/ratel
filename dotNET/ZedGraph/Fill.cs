...
namespace ZedGraph {
    ...
    [Serializable] public class Fill : ISerializable, ICloneable {
        #region Fields
        /// <summary>
        ...
        /// </summary>
        Color _secondaryValueGradientColor;
        /// <summary>
        ...
        public Fill(Color color) {
            Init();
            _color = color;
            if (color != Color.Empty) _type = FillType.Solid;
        }
        ...
        public Fill(Color[] colors, float angle) {
            ...
            var blend = new ColorBlend {Colors = colors, Positions = new float[colors.Length]};
            blend.Positions[0] = 0.0F;
            for (var i = 1; i < colors.Length; i++) blend.Positions[i] = i / (float) (colors.Length - 1);
            _type = FillType.Brush;
        ...
        public Fill(ICloneable brush) : this(brush, Default.IsScaled) {}
        ...
        public Fill(ICloneable brush, bool isScaled) {
            Init();
            _isScaled = isScaled;
            _color = Color.White;
        ...
        public Fill(ICloneable brush, AlignH alignH, AlignV alignV) {
            Init();
            _alignH = alignH;
            _alignV = alignV;
            _isScaled = false;
        ...
        public Fill(Fill rhs) {
            ...
            if (rhs._brush != null) _brush = (Brush) rhs._brush.Clone();
            else _brush = null;
            _type = rhs._type;
            _alignH = rhs.AlignH;
            _alignV = rhs.AlignV;
            ...
            if (rhs._colorList != null) _colorList = (Color[]) rhs._colorList.Clone();
            else _colorList = null;

            if (rhs._positionList != null) _positionList = (float[]) rhs._positionList.Clone();
            else _positionList = null;

            if (rhs._image != null) _image = (Image) rhs._image.Clone();
            else _image = null;
        ...
        void CreateBrushFromBlend(ColorBlend blend, float angle) {
            _angle = angle;
        ...
        protected Fill(SerializationInfo info, StreamingContext context) {
            ...
            if (_colorList != null && _positionList != null) {
                var blend = new ColorBlend {Colors = _colorList, Positions = _positionList};
                CreateBrushFromBlend(blend, _angle);
            } else if (_image != null) _brush = new TextureBrush(_image, _wrapMode);
        ...
        public Brush MakeBrush(RectangleF rect, PointPair dataValue) {
            ...
            if (IsVisible && (!_color.IsEmpty || _brush != null)) {
                ...
                //Brush	brush;
                if (_type == FillType.Brush) return ScaleBrush(rect, _brush, _isScaled);
                if (IsGradientValueType)
                    if (dataValue != null)
                        if (!_secondaryValueGradientColor.IsEmpty) {
                            ...
                            var tmpFill = new Fill(
                                _secondaryValueGradientColor,
                                GetGradientColor(dataValue),
                                _angle);
                            return tmpFill.MakeBrush(rect);
                        } else return new SolidBrush(GetGradientColor(dataValue));
                        ...
                        if (!_secondaryValueGradientColor.IsEmpty) {
                            ...
                            var tmpFill = new Fill(
                                _secondaryValueGradientColor,
                                GetGradientColor(_rangeDefault),
                                _angle);
                            return tmpFill.MakeBrush(rect);
                        } else return new SolidBrush(GetGradientColor(_rangeDefault));
                    else return ScaleBrush(rect, _brush, true);
        ...
        internal Color GetGradientColor(PointPair dataValue) {
            ...
            if (dataValue == null) val = _rangeDefault;
            else switch (_type) {
                case FillType.GradientByColorValue: val = dataValue.ColorValue; break;
                case FillType.GradientByZ: val = dataValue.Z; break;
                case FillType.GradientByY: val = dataValue.Y; break;
                default: val = dataValue.X; break;
            }
        ...
        internal Color GetGradientColor(double val) {
            ...
            if (Double.IsInfinity(val) || double.IsNaN(val) || val == PointPairBase.Missing) val = _rangeDefault;

            if (_rangeMax - _rangeMin < 1e-20 || val == double.MaxValue) valueFraction = 0.5;
            else valueFraction = (val - _rangeMin) / (_rangeMax - _rangeMin);

            if (valueFraction < 0.0) valueFraction = 0.0;
            else if (valueFraction > 1.0) valueFraction = 1.0;
        ...
        Brush ScaleBrush(RectangleF rect, ICloneable brush, bool isScaled) {
            if (brush != null)
                if (brush is SolidBrush) return (Brush) brush.Clone();
        ...
        public void Draw(Graphics g, RectangleF rect, PointPair pt) {
            if (IsVisible) using (var brush = MakeBrush(rect, pt)) g.FillRectangle(brush, rect);
        }
        #endregion
    }
}