...
namespace ZedGraph {
    ...
    [Serializable] public class ErrorBar : ICloneable, ISerializable {
        #region Fields
        /// <summary>
        ...
        public ErrorBar(Color color) {
            _symbol = new Symbol(Default.Type, color) {Size = Default.Size};
            _color = color;
        ...
        public void Draw(
            ...
            if (curve.Points != null && IsVisible)
                using (var pen = !curve.IsSelected
                    ...
                    :
                        new Pen(Selection.Border.Color, Selection.Border.Width)) // Loop over each defined point							
                    for (var i = 0; i < curve.Points.Count; i++) {
                        double scaleBase;
                        double scaleValue;
                        double scaleLowValue;
                        ...
                        if (curve.Points[i].IsInvalid3D || (scaleBase <= 0 && baseAxis._scale.IsLog) ||
                            ((scaleValue <= 0 || scaleLowValue <= 0) && valueAxis._scale.IsLog)) continue;
                        var pixBase = baseAxis.Scale.Transform(curve.IsOverrideOrdinal, i, scaleBase);
                        var pixValue = valueAxis.Scale.Transform(curve.IsOverrideOrdinal, i, scaleValue);
                        var pixLowValue = valueAxis.Scale.Transform(curve.IsOverrideOrdinal, i, scaleLowValue);
