...
namespace ZedGraph {
    ...
    [Serializable] public abstract class Axis : ISerializable, ICloneable {
        #region Class Fields
        ...
        internal void DrawGrid(Graphics g, GraphPane pane, float scaleFactor, float shiftPos) {
            ...
            var baseVal = _scale.CalcBaseTic();
        ...
        public void SetMinSpaceBuffer(
            ...
            if (isGrowOnly) space = Math.Max(oldSpace, space);
            // Set the minSpace
            MinSpace = space;
        }
...
//		abstract internal float GetMinPix( GraphPane pane );
        ...
        internal double EffectiveCrossValue(GraphPane pane) {
            ...
            var min = crossAxis._scale.Linearize(crossAxis._scale._min);
            var max = crossAxis._scale.Linearize(crossAxis._scale._max);

            if (_crossAuto) return crossAxis._scale.IsReverse == (this is Y2Axis || this is X2Axis) ? max : min;
            if (_cross < min) return min;
            return _cross > max ? max : _scale.Linearize(_cross);
        }
        ...
        internal bool IsCrossShifted(GraphPane pane) {
            if (_crossAuto) return false;
            var crossAxis = GetCrossAxis(pane);
            if (((this is XAxis || this is YAxis) && !crossAxis._scale.IsReverse) ||
                ((this is X2Axis || this is Y2Axis) && crossAxis._scale.IsReverse)) {
                if (_cross <= crossAxis._scale._min) return false;
            } else if (_cross >= crossAxis._scale._max) return false;
        ...
        internal float CalcCrossFraction(GraphPane pane) {
            ...
            if (!IsCrossShifted(pane)) {
                ...
                if (IsPrimary(pane) && _scale._isLabelsInside) return 1.0f;
            ...
            var max = crossAxis._scale.Linearize(crossAxis._scale._min);
            var min = crossAxis._scale.Linearize(crossAxis._scale._max);
            float frac;

            if (((this is XAxis || this is YAxis) && _scale._isLabelsInside == crossAxis._scale.IsReverse) ||
                ((this is X2Axis || this is Y2Axis) && _scale._isLabelsInside != crossAxis._scale.IsReverse)) frac = (float) ((effCross - min) / (max - min));
            else frac = (float) ((max - effCross) / (max - min));
        ...
        float CalcTotalShift(GraphPane pane, float scaleFactor, float shiftPos) {
            if (!IsPrimary(pane)) // if ( CalcCrossFraction( pane ) != 0.0 )
                if (IsCrossShifted(pane)) shiftPos = 0;
                else {
                    ...
                    if (_scale._isLabelsInside) {
                        ...
                        if (_majorTic.IsOutside || _majorTic._isCrossOutside ||
                            _minorTic.IsOutside || _minorTic._isCrossOutside) shiftPos -= ticSize;
                    } else {
        ...
        public float CalcSpace(Graphics g, GraphPane pane, float scaleFactor, out float fixedSpace) {
            ...
            var charHeight = _scale._fontSpec.GetHeight(scaleFactor);
            // Scaled size (pixels) of a tic
            var ticSize = _majorTic.ScaledTic(scaleFactor);
            // Scaled size (pixels) of the axis gap
            var axisGap = _axisGap * scaleFactor;
            var scaledLabelGap = _scale._labelGap * charHeight;
            var scaledTitleGap = _title.GetScaledGap(scaleFactor);
            ...
            if (_isVisible) {
                ...
                if (!IsPrimary(pane)) {
                    ...
                    if (MajorTic._isInside || MajorTic._isCrossInside ||
                        MinorTic._isInside || MinorTic._isCrossInside) _tmpSpace += ticSize;
                }
            ...
            if (IsPrimary(pane) && ((
                ...
                var tmp = pane.XAxis._scale.GetScaleMaxSpace(g, pane, scaleFactor, true).Width / 2.0F;
                //+ charHeight / 2.0F;
                //if ( tmp > tmpSpace )
                //	tmpSpace = tmp;
        ...
        internal void FixZeroLine(
            ...
            if (!_isVisible || !_majorGrid._isZeroLine || _scale._min >= 0.0 || _scale._max <= 0.0) return;
            var zeroPix = _scale.Transform(0.0);

            using (var zeroPen = new Pen(
                _color,
                pane.ScaledPenWidth(_majorGrid._penWidth, scaleFactor))) g.DrawLine(zeroPen, left, zeroPix, right, zeroPix);
                //zeroPen.Dispose();
        }
        ...
        public void DrawMinorTics(
            ...
            float scaleFactor,
            float topPix) {
            // bug fixMe variable removed, is this known to be broken?
            if ((((((!MinorTic.IsOutside && !MinorTic.IsOpposite) && !MinorTic.IsInside) &&
                !MinorTic._isCrossOutside) && !MinorTic._isCrossInside) && !_minorGrid._isVisible) || !_isVisible) return;
            double tMajor = _scale._majorStep * _scale.MajorUnitMultiplier,
                tMinor = _scale._minorStep * _scale.MinorUnitMultiplier;

            if (!_scale.IsLog && tMinor >= tMajor) return;
            ...
            var iTic = _scale.CalcMinorStart(baseVal);
            var majorTic = 0;
            var majorVal = _scale.CalcMajorTicValue(baseVal, majorTic);

            using (var pen = new Pen(
                _minorTic._color,
                pane.ScaledPenWidth(MinorTic._penWidth, scaleFactor)))
            using (var minorGridPen = _minorGrid.GetPen(pane, scaleFactor)) // Draw the minor tic marks
                while (dVal < last && iTic < 5000) {
                    ...
                    if (dVal > majorVal) majorVal = _scale.CalcMajorTicValue(baseVal, ++majorTic);
                    ...
                    if (((Math.Abs(dVal) < 1e-20 && Math.Abs(dVal - majorVal) > 1e-20) ||
                        ...
                        var pixVal = _scale.LocalTransform(dVal);
        ...
        public void DrawTitle(Graphics g, GraphPane pane, float shiftPos, float scaleFactor) {
            ...
            if (!_isVisible || !_title._isVisible || string.IsNullOrEmpty(str)) return;
            var hasTic = _scale._isLabelsInside
                ? MajorTic.IsInside || MajorTic._isCrossInside || MinorTic.IsInside || MinorTic._isCrossInside
            ...
            var x = (_scale._maxPix - _scale._minPix) / 2;

            var scaledTic = MajorTic.ScaledTic(scaleFactor);
            var scaledLabelGap = _scale._fontSpec.GetHeight(scaleFactor) * _scale._labelGap;
            var scaledTitleGap = _title.GetScaledGap(scaleFactor);
            ...
            var y = (_scale._isVisible
                ? _scale.GetScaleMaxSpace(g, pane, scaleFactor, true).Height
                    + scaledLabelGap
                : 0);

            if (_scale._isLabelsInside) y = shiftPos - y - gap;
            else y = shiftPos + y + gap;

            if (!_crossAuto && !_title._isTitleAtCross) y = Math.Max(y, gap);
            ...
            // Draw the title
            Title.FontSpec.Draw(g, pane, str, x, y, AlignH.Center, alignV, scaleFactor);
        }

        string MakeTitle() {
            if (_title._text == null) _title._text = "";
            ...
            if (_scale._mag != 0 && !_title._isOmitMag && !_scale.IsLog) return _title._text + String.Format(" (10^{0})", _scale._mag);
            return _title._text;
        }
        ...
        internal string MakeLabelEventWorks(GraphPane pane, int index, double dVal) {
            ...
            return Scale != null ? _scale.MakeLabel(pane, index, dVal) : "?";
        }
        #endregion
    }
}