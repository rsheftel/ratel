...
namespace ZedGraph {
    ...
    [Serializable] public class GraphPane : PaneBase, ICloneable, ISerializable {
        #region Events
        #region Delegates
        /// <summary>
        ...
        readonly Y2AxisList _y2AxisList;
        ...
        /// </summary>
        readonly ZoomStateStack _zoomStack;
        ...
        [Bindable(true), Browsable(true), Category("Display"), NotifyParentProperty(true),
         Description("Determines whether the auto-ranged scale will include all data points" +
             " or just the visible data points")] public bool IsIgnoreInitial {
        ...
        protected GraphPane(SerializationInfo info, StreamingContext context)
            ...
            if (sch >= 11) _x2Axis = (X2Axis) info.GetValue("x2Axis", typeof (X2Axis));
            else _x2Axis = new X2Axis("");
        ...
        public void AxisChange() {
            using (var g = Graphics.FromHwnd(IntPtr.Zero)) AxisChange(g);
        }
        ...
        public void AxisChange(Graphics g) {
            ...
            if (_barSettings._clusterScaleWidthAuto) _barSettings._clusterScaleWidth = 1.0;
            ...
            if (AxisChangeEvent != null) AxisChangeEvent(this);
        }

        void PickScale(Graphics g, float scaleFactor) {
            var maxTics = 0;
            ...
            foreach (var axis in _yAxisList) {
                ...
                var nTics = axis._scale.CalcNumTics();
            ...
            foreach (var axis in _yAxisList) ForceNumTics(axis, maxTics);
            foreach (var axis in _y2AxisList) ForceNumTics(axis, maxTics);
        }

        static void ForceNumTics(Axis axis, int numTics) {
            if (!axis._scale.MaxAuto) return;
            var nTics = axis._scale.CalcNumTics();
            if (nTics < numTics) axis._scale._maxLinearized += axis._scale._majorStep * (numTics - nTics);
        }
        ...
        public override void Draw(Graphics g) {
            ...
            foreach (var axis in _yAxisList) axis.Scale.SetupScaleData(this, axis);
            foreach (var axis in _y2AxisList) axis.Scale.SetupScaleData(this, axis);

            // Draw the GraphItems that are behind the Axis objects
            if (showGraf) _graphObjList.Draw(g, this, scaleFactor, ZOrder.G_BehindChartFill);
        ...
        bool AxisRangesValid() {
            var showGraf = _xAxis._scale._min < _xAxis._scale._max &&
                _x2Axis._scale._min < _x2Axis._scale._max;
            foreach (var axis in _yAxisList) if (axis._scale._min >= axis._scale._max) showGraf = false;
            foreach (var axis in _y2AxisList) if (axis._scale._min >= axis._scale._max) showGraf = false;
        ...
        public RectangleF CalcChartRect(Graphics g, float scaleFactor) {
            ...
            foreach (var axis in _yAxisList) {
                ...
                if (axis.IsCrossShifted(this)) totSpaceY += tmp;
...
//			minSpaceT = Math.Max( minSpaceT, spaceT );
        ...
        void SetSpace(Axis axis, float clientSize, ref float spaceNorm, ref float spaceAlt) {
            //spaceNorm = 0;
            //spaceAlt = 0;
            ...
            if (!axis.IsPrimary(this) && axis.IsCrossShifted(this)) axis._tmpSpace = 0;

            if (axis._tmpSpace < crossPix) axis._tmpSpace = 0;
            else if (crossPix > 0) axis._tmpSpace -= crossPix;

            if (axis._scale._isLabelsInside && (axis.IsPrimary(this) || (crossFrac != 0.0 && crossFrac != 1.0))) spaceAlt = axis._tmpSpace;
            else spaceNorm = axis._tmpSpace;
        }
        ...
        public void SetMinSpaceBuffer(Graphics g, float bufferFraction, bool isGrowOnly) {
            ...
            foreach (var axis in _yAxisList) axis.SetMinSpaceBuffer(g, this, bufferFraction, isGrowOnly);
            foreach (var axis in _y2AxisList) axis.SetMinSpaceBuffer(g, this, bufferFraction, isGrowOnly);
        }
        #endregion
        ...
        public ErrorBarItem AddErrorBar(
            ...
            double[] baseValue,
            Color color) {
            var curve = new ErrorBarItem(
                label,
                new PointPairList(x, y, baseValue),
                color);
            _curveList.Add(curve);
        ...
        public void ReverseTransform(
            ...
            // Setup the scaling data based on the chart rect
            Axis xAxis = _xAxis;
            if (isX2Axis) xAxis = _x2Axis;

            xAxis.Scale.SetupScaleData(this, xAxis);
            x = xAxis.Scale.ReverseTransform(ptF.X);

            Axis yAxis = null;
            if (isY2Axis && Y2AxisList.Count > yAxisIndex) yAxis = Y2AxisList[yAxisIndex];
            else if (!isY2Axis && YAxisList.Count > yAxisIndex) yAxis = YAxisList[yAxisIndex];

            if (yAxis != null) {
                yAxis.Scale.SetupScaleData(this, yAxis);
                y = yAxis.Scale.ReverseTransform(ptF.Y);
            } else y = PointPairBase.Missing;
        }
        ...
        public bool FindNearestObject(
            ...
            if (AxisRangesValid()) {
                ...
                var paneTitleBox = _title._fontSpec.BoundingBox(g, _title._text, scaleFactor);
                if (saveZOrder <= ZOrder.H_BehindAll && _title._isVisible) {
                    tmpRect = new RectangleF(
                        (_rect.Left + _rect.Right - paneTitleBox.Width) / 2,
                ...
                tmpRect = new RectangleF(
                    ...
                    tmpChartRect.Bottom,
                    tmpChartRect.Width,
                    height); //_rect.Bottom - tmpChartRect.Bottom );
        ...
        public bool FindNearestPoint(
            ...
            if (!_chart._rect.Contains(mousePt)) return false;
            ...
            if (!AxisRangesValid()) return false;
            ...
            foreach (var curve in targetCurveList) //test for pie first...if it's a pie rest of method superfluous
                if (curve is PieItem && curve.IsVisible) {
                    ...
                    var yPixPerUnitAct = _chart._rect.Height / (yMaxAct - yMinAct);

                    var xPixPerUnit = _chart._rect.Width / (xAxis._scale._max - xAxis._scale._min);
                    var xAct = xAxis is XAxis ? x : x2;
                    ...
                    if (isXBaseAxis) barWidthUserHalf = barWidth / xPixPerUnit / 2.0;
                    else barWidthUserHalf = barWidth / yPixPerUnitAct / 2.0;

                    if (points != null) {
                        for (var iPt = 0; iPt < curve.NPts; iPt++) {
                            // xVal is the user scale X value of the current point
                            double xVal;
                            if (xAxis._scale.IsAnyOrdinal && !curve.IsOverrideOrdinal) xVal = iPt + 1.0;
                            else xVal = points[iPt].X;

                            // yVal is the user scale Y value of the current point
                            double yVal;
                            if (yAxis._scale.IsAnyOrdinal && !curve.IsOverrideOrdinal) yVal = iPt + 1.0;
                            else yVal = points[iPt].Y;
                            ...
                            if (xVal != PointPairBase.Missing &&
                                ...
                                if (curve.IsBar || curve is ErrorBarItem ||
                                    ...
                                    if (isXBaseAxis) {
                                        ...
                                        if (xAct < centerVal - barWidthUserHalf ||
                                            xAct > centerVal + barWidthUserHalf ||
                                                yAct < lowVal || yAct > hiVal) continue;
                                    } else {
                                        ...
                                        if (yAct < centerVal - barWidthUserHalf ||
                                            yAct > centerVal + barWidthUserHalf ||
                                                xAct < lowVal || xAct > hiVal) continue;
                                    }
                                    ...
                                    var distX = (xVal - xAct) * xPixPerUnit;
                                    var distY = (yVal - yAct) * yPixPerUnitAct;
            ...
            if (nearestCurve is LineItem) {
                var halfSymbol = (((LineItem) nearestCurve).Symbol.Size *
                    CalcScaleFactor() / 2);
                    ...
                    if (ci.Points[i].X > rectF.Left &&
                        ci.Points[i].X < rectF.Right &&
                            ci.Points[i].Y > rectF.Bottom &&
                                ci.Points[i].Y < rectF.Top) containedObjs.Add(ci);
