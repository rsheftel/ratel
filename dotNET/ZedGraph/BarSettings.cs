...
namespace ZedGraph {
    ...
    [Serializable] public class BarSettings : ISerializable {
        #region Fields
...
//				else if ( curve is JapaneseCandleStickItem &&
        ...
        internal static double GetMinStepSize(IPointList list, Axis baseAxis) {
            ...
            if (list.Count <= 0 || baseAxis._scale.IsAnyOrdinal) return 1.0;
            ...
            for (var i = 1; i < list.Count; i++) {
                ...
                if (!pt.IsInvalid || !lastPt.IsInvalid) {
                    double step;
                    if (baseAxis is XAxis || baseAxis is X2Axis) step = pt.X - lastPt.X;
                    else step = pt.Y - lastPt.Y;
            ...
            if (range <= 0) minStep = 1.0;
//			else if ( minStep <= 0 || minStep < 0.001 * range || minStep > range )
            else if (minStep <= 0 || minStep > range) minStep = 0.1 * range;
