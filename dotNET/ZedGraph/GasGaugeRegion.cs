...
namespace ZedGraph {
    ...
    [Serializable] public class GasGaugeRegion : CurveItem, ICloneable {
        #region Fields
        /// <summary>
...
//			public static PieLabelType LabelType = PieLabelType.Name;
        ...
        public static void CalculateGasGuageParameters(GraphPane pane) {
            ...
            foreach (var curve in pane.CurveList)
                if (curve is GasGaugeRegion) {
                    var ggr = (GasGaugeRegion) curve;
                    var start = ((float) ggr.MinValue - (float) minVal) / ((float) maxVal - (float) minVal) * 180.0f;
                    var sweep = ((float) ggr.MaxValue - (float) minVal) / ((float) maxVal - (float) minVal) * 180.0f;
                    sweep = sweep - start;

                    var f = new Fill(Color.White, ggr.RegionColor, -(sweep / 2f));
                    ggr.Fill = f;
