...
namespace ZedGraph {
    ...
    [Serializable] public class Bar : ICloneable, ISerializable {
        #region Fields
        /// <summary>
        ...
        public struct Default {
            ...
            /// </summary>
            public static Brush FillBrush; //new LinearGradientBrush( new Rectangle(0,0,100,100),
            /// <summary>
        ...
        public void Draw(
            ...
            else if (top > 10000) top = 10000;
            if (left < -10000) left = -10000;
            else if (left > 10000) left = 10000;
            if (right < -10000) right = -10000;
            else if (right > 10000) right = 10000;
            if (bottom < -10000) bottom = -10000;
            else if (bottom > 10000) bottom = 10000;
        ...
        public void DrawBars(
            ...
            if (barType == BarType.Overlay || barType == BarType.Stack || barType == BarType.PercentStack ||
                barType == BarType.SortedOverlay) pos = 0;

            // Loop over each defined point and draw the corresponding bar                
            for (var i = 0; i < curve.Points.Count; i++) DrawSingleBar(g, pane, curve, i, pos, baseAxis, valueAxis, barWidth, scaleFactor);
        }
        ...
        public void DrawSingleBar(
            ...
            if (index >= curve.Points.Count) return;
            ...
            if (pane._barSettings.Type == BarType.Overlay || pane._barSettings.Type == BarType.Stack ||
                pane._barSettings.Type == BarType.PercentStack) pos = 0;
        ...
        protected virtual void DrawSingleBar(
            ...
            if (curve.Points[index].IsInvalid) return;
            // calculate a pixel value for the top of the bar on value axis
            var pixLowVal = valueAxis.Scale.Transform(curve.IsOverrideOrdinal, index, curLowVal);
            var pixHiVal = valueAxis.Scale.Transform(curve.IsOverrideOrdinal, index, curHiVal);
            // calculate a pixel value for the center of the bar on the base axis
            var pixBase = baseAxis.Scale.Transform(curve.IsOverrideOrdinal, index, curBase);
