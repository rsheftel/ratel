...
namespace ZedGraph {
    ...
    [Serializable] public class OHLCBar : LineBase, ICloneable {
        #region Fields
        /// <summary>
        ...
        public void Draw(
            ...
            if (pixBase != PointPairBase.Missing)
                if (isXBase) {
                    if (Math.Abs(pixLow) < 1000000 && Math.Abs(pixHigh) < 1000000) g.DrawLine(pen, pixBase, pixHigh, pixBase, pixLow);
                    if (_isOpenCloseVisible && Math.Abs(pixOpen) < 1000000) g.DrawLine(pen, pixBase - halfSize, pixOpen, pixBase, pixOpen);
                    if (_isOpenCloseVisible && Math.Abs(pixClose) < 1000000) g.DrawLine(pen, pixBase, pixClose, pixBase + halfSize, pixClose);
                } else {
                    if (Math.Abs(pixLow) < 1000000 && Math.Abs(pixHigh) < 1000000) g.DrawLine(pen, pixHigh, pixBase, pixLow, pixBase);
                    if (_isOpenCloseVisible && Math.Abs(pixOpen) < 1000000) g.DrawLine(pen, pixOpen, pixBase - halfSize, pixOpen, pixBase);
                    if (_isOpenCloseVisible && Math.Abs(pixClose) < 1000000) g.DrawLine(pen, pixClose, pixBase, pixClose, pixBase + halfSize);
                }
        }
        ...
        public void Draw(
            ...
            using (var pen = !curve.IsSelected
                ...
                :
                    new Pen(Selection.Border.Color, Selection.Border.Width)) //				using ( Pen pen = new Pen( _color, _penWidth ) )
        ...
        public float GetBarWidth(GraphPane pane, Axis baseAxis, float scaleFactor) {
            ...
            else width = (_size * scaleFactor) / 2.0f;
