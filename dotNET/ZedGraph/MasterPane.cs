...
namespace ZedGraph {
    ...
    [Serializable] public class MasterPane : PaneBase, ICloneable, ISerializable, IDeserializationCallback {
        #region Fields
        /// <summary>
...
*/
        ...
        /// </param>
        protected MasterPane(SerializationInfo info, StreamingContext context) : base(info) {
            ...
            if (sch >= 11) _isAntiAlias = info.GetBoolean("isAntiAlias");
        }
        ...
        public void AxisChange(Graphics g) {
            foreach (var pane in _paneList) pane.AxisChange(g);
        }
        ...
        public void CommonScaleFactor() {
            if (!_isCommonScaleFactor) return;
        ...
        public override void Draw(Graphics g) {
            ...
            foreach (var pane in _paneList) pane.Draw(g);
        ...
        public GraphPane FindPane(PointF mousePt) {
            foreach (var pane in _paneList) if (pane.Rect.Contains(mousePt)) return pane;
        ...
        public GraphPane FindChartRect(PointF mousePt) {
            foreach (var pane in _paneList) if (pane.Chart._rect.Contains(mousePt)) return pane;
        ...
        public void SetLayout(Graphics g, int rows, int columns) {
            ...
            for (var i = 0; i < rows; i++) countList[i] = columns;
        ...
        public void SetLayout(Graphics g, bool isColumnSpecified, int[] countList, float[] proportion) {
            ...
            for (var i = 0; i < countList.Length; i++) _prop[i] /= sumProp;
            ...
            if (_countList != null) DoLayout(g, _isColumnSpecified, _countList, _prop);
                ...
                switch (_paneLayout) {
                    ...
                    default:
                        rows = root;
                        cols = root;
                        if (count <= root * (root - 1)) rows--;
                        DoLayout(g, rows, cols);
                        break;
                    case PaneLayout.SquareRowPreferred:
                        rows = root;
                        cols = root;
                        if (count <= root * (root - 1)) cols--;
                        DoLayout(g, rows, cols);
                        break;
                    case PaneLayout.ExplicitCol12:
                        DoLayout(g, true, new[] {1, 2}, null);
                        break;
                    case PaneLayout.ExplicitCol21:
                        DoLayout(g, true, new[] {2, 1}, null);
                        break;
                    case PaneLayout.ExplicitCol23:
                        DoLayout(g, true, new[] {2, 3}, null);
                        break;
                    case PaneLayout.ExplicitCol32:
                        DoLayout(g, true, new[] {3, 2}, null);
                        break;
                    case PaneLayout.ExplicitRow12:
                        DoLayout(g, false, new[] {1, 2}, null);
                        break;
                    case PaneLayout.ExplicitRow21:
                        DoLayout(g, false, new[] {2, 1}, null);
                        break;
                    case PaneLayout.ExplicitRow23:
                        DoLayout(g, false, new[] {2, 3}, null);
                        break;
                    case PaneLayout.ExplicitRow32:
                        DoLayout(g, false, new[] {3, 2}, null);
                        break;
                }
            }
        }
            ...
            if (isColumnSpecified) {
                ...
                for (var rowNum = 0; rowNum < rows; rowNum++) {
                    ...
                    for (var colNum = 0; colNum < columns; colNum++) {
                        if (iPane >= _paneList.Count) return;
