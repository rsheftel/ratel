...
import static org.ratel.r.reflect.RatelParameter.*;
...
public class RGenerator {
    ...
    class RBuilder<T> {
        ...
        public void addConstructorAndMethodsAndFields() {
            addConstructors();
            List<RatelMethod> methods = q.methods();
            for (RatelMethod method : methods)
                addMethod(method);
            addFields();
        }

        private void addFields() {
            List<RatelField> fields = q.fields();
            for (RatelField field : fields)
                addMethod(
        ...
        private RCode fieldDeclaration(RatelField field) {
        ...
        private void addMethod(RatelMethod method) {
            ...
            for (RatelParameter p : method.parameters())
        ...
        private RCode parameterCode(RatelParameter parameter) {
        ...
        private RCode parameterCodeArray(RatelParameter parameter, RawR pName) {
            String pType = parameter.type().qualifiedName();
            if (!isObject(pType)) 
        ...
        private RCode asJArray(RatelParameter p, RCode rArray, String pType) {
        ...
        private Call wrapInJRObjectIfNeeded(RatelType type, Call jcall) {
        ...
        private String methodName(RatelMethod method) {
        ...
        private void addConstructors() {
            ...
            for (RatelMethod constructor : q.constructors())
                addConstructor(constructor);
        }

        private void addConstructor(RatelMethod constructor) {
            ...
            for (RatelParameter p : constructor.parameters())
        ...
        private String methodSuffix(RatelMethod method) {
            List<String> parts = RatelParameter.simpleTypes(method.parameters());
            parts.add(0, "by");
            return join("_", parts);
        }
        ...
        private RCode jniName(RatelType type) {
            // Stolen from
        ...
        private List<RCode> methodParams(RatelMethod constructor) {
            return params(constructor);
        }

        private List<RCode> params(RatelMethod method) {
    ...
    public <T> List<String> allRCode(Class<T> c) {
        List<Class<?>> classes = RatelClass.allClassesInPackage(c);
