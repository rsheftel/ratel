...
testUNCPathsBorkWithGoodErrorMessage <- function() {
    dir = tempdir()
    shouldBombMatching(copyDirectory(dir, "//a"), "Please use a path like V:/foo instead of //nyux51/data/foo")
}
...
test.fileMatches <- function(){
	testDataPath <- squish(system.file("testdata", package="GSFCore"),'/')
