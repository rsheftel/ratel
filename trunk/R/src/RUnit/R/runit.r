...
stackTraceErrorMessage <- function(calls, func.original, func.local) {
    calls <- rev(calls)[-(1:2)]
    tb <- lapply(calls, deparse)
    func.index <- match(func.local, tb)
    tb[func.index] <- paste(func.original, "()", sep = "")
    tb <- tb[1:func.index]
    frame.names <- capture.output(traceback(tb))
    stack.trace <- paste("   ", frame.names, collapse = "\n")
    error.message <- paste(
        "\n========================================\n", 
        geterrmessage(),
        "\n\nStack Trace:\n", 
...
runTestErrorHandler <- function(e, calls, logger, funcName) {
    error.message <- stackTraceErrorMessage(calls, funcName, "func()")

    if(logger$isFailure()) {
      logger$addFailure(testFuncName=funcName, failureMsg=error.message)
    } else if(logger$isDeactivated()) {
      logger$addDeactivated(testFuncName=funcName)
      return()
    } else {
      logger$addError(testFuncName=funcName, errorMsg=error.message)
    }
...
# refactor me
errorHandlerInSetUp <- function(e, calls, logger, funcName) {
    funcName <- paste(".setUp (before ", funcName, ")", sep="")
    error.message <- stackTraceErrorMessage(calls, funcName, "setUpFunc()")

    .testLogger$addError(testFuncName=funcName, errorMsg=error.message)
    message(error.message)
}

errorHandlerInTearDown <- function(e, calls, logger, funcName) {
    funcName <- paste(".tearDown (before ", funcName, ")", sep="")
    error.message <- stackTraceErrorMessage(calls, funcName, "tearDownFunc()")
...
.executeTestCase <- function(funcName, envir, setUpFunc, tearDownFunc) {
  try.withRecovery <- function(runMe, errorHandler) {
    try(
        withRestarts(
            withCallingHandlers(
                runMe(),
                error = function(e) { 
                    ...
                    invokeRestart("grumble", e, sys.calls()) 
                }
            ), 
            grumble = function(e, calls) errorHandler(e, calls, .testLogger, funcName)
        )
    )
  }

  func <- get(funcName, envir=envir)
  if(mode(func) != "function") return() 

  cat("test:",funcName, "\n")

  res <- try.withRecovery(function() { setUpFunc(); "setUpSucceeded" }, errorHandlerInSetUp)
  if (is.null(res)) return()

  .testLogger$cleanup()

  # RUN TEST
  timing <- try.withRecovery(function() system.time(func()), runTestErrorHandler)
  if (inherits(timing, "proc_time"))
    .testLogger$addSuccess(testFuncName=funcName, secs=round(timing[3], 2))

  try.withRecovery(function() tearDownFunc(), errorHandlerInTearDown)
...
runTestSuite <- function(testSuites, useOwnErrorHandler=TRUE) {
  ...
  for (i in seq(length=length(testSuites))) {
    ...
    testFiles <- unlist(sapply(testSuite$dirs, function(dir) list.files(dir,
