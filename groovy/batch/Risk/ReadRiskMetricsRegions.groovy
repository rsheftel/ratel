 ...
 *
 */
class ReadRiskMetricsRegions {

    private static int HEADER_ROW = 7;

    private static String TABLE_NAME = "RiskMetrixRegion";
    // Special column names
    private static String[][] COLUMNS = [["ACCOUNT", "COMPOSIT"],
            ["Ticker", "Ticker"], ["Country", "COMPOSIT"]
    ];

    private static List<String[]> fileToDbColumnNames = new ArrayList<String[]>();

    static void main(args) {
        def cli = new CliBuilder(usage: 'groovy ReadRiskMetrics -d dir -f filename [-o output dir]')
        ...
        srcFile.splitEachLine("\t") {
            ...
            if (lineCount == HEADER_ROW) {
                ...
                for (columnName in lineParts) {
                    ...
                    String dbColumnName = findSpecialColumnName(columnName);
                    if (dbColumnName == "") {
                        ...
                        dbColumnIndex++;
                    } else if (dbColumnName == "COMPOSIT") {
                        dbColumnIndex = addCompositColumn(dbColumnIndex, columnName, dbColumnIndexMapping);
                    } else {
                        dbColumnIndexMapping.put(dbColumnIndex, dbColumnName);
                        dbColumnIndex++;
                    }
                }
                // We have a constant column to add
                dbColumnIndexMapping.put(dbColumnIndex, "usc");
                uscIndex = dbColumnIndex;
                ...
                for (String columnData in lineParts) {
                    ...
                    if (columnData.contains("Problems Encountered During Processing")) {
                        ...
                        if (columnName != null && columnName.trim().length() > 0) {
                            // Process this column
                            //println("process " + columnData + " for column " + columnName);
//                            if (columnData.trim().length() > 50) {
//                                System.out.println(columnName+" has length of "+columnData.trim().length())
//                            }
//                            try {
//                                BigDecimal test = new BigDecimal(columnData.trim());
//                                if (test.precision() + test.scale() > 15) {
//                                    System.out.println(columnName+" has precision + scale of "+(test.precision() + test.scale()));
//                                }
//                            } catch (Exception e) { }
                            if (columnName == "Ticker") {
                                if (isPorfolioLine(columnData)) {
                                    portfolio = columnData.trim();
                                } else if (columnData.length() > 1 && columnData.charAt(0) != ' ') {
                                    portfolio = null;
                                }

                                if (isUnsettledCloseLine(columnData)) {
                                    usc = "Y";
                                } else if (noLongerUnsettledClose(columnData)) {
                                    usc = "N";
                                }
                                dbColumnData.add(columnData.trim());

                            } else if (columnName == "ACCOUNT") {
                                splitAccount(dbColumnData, columnData);
                            } else if (columnName == "Country") {
                                splitCountry(dbColumnData, columnData);
                            } else if (columnName == "Portfolio" && portfolio != null) {
                       ...
                       // USC is a madeup column, put it at the end
                        if (dbColumnData.size() == uscIndex) {
                            dbColumnData.add(usc); // This is the last column
                        }
                        columnIndex++;
                    }
                }
                ...
                if (dbColumnData.size() > 1) {
                    ...
                    //println("Number of columns read=" + dbColumnData.size());
                }
            } else {
...
// Add the date to the end
        ...
        if (dbADRow != null && dbADRow.containsKey("AnalysisDate")) {
            ...
            //throw new IllegalArgumentException("Date already loaded " + analysisDate);
            System.exit(1);
        }
...
// This is the end of the file - spit out the data
    ...
    // A portfolio line has only 1 leading space
    ...
    private static String findSpecialColumnName(String spreadSheetColumnName) {
        for (String[] entry in COLUMNS) {
            if (entry[0] == spreadSheetColumnName) {
                return entry[1];
            }
        }
        return "";
