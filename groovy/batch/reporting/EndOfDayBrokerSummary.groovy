 ...
 * 
 */
public class EndOfDayBrokerSummary {
    ...
    static final String SQL_GROUP_FIELDS = "SenderCompId, TradeDate, ExecutingBroker, Symbol"
    ...
    public static void main(args) {
        ...
        for (orderSummary in results) {
            def orderKey = orderSummary.SenderCompId + orderSummary.TradeDate + orderSummary.ExecutingBroker +
                    orderSummary.Symbol
        ...
        reportBody.append("\n\nBroker\tSymbol\tBuys\tSells\tNet\tBuy AvgP\tSell AvgP")
        reportBody.append("\n")
        ...
        for (reportLine in orderReport) {
            ...
            def newBroker = entry.get("Broker")
            if (newBroker == null || newBroker == "null") {
                newBroker = "Unknown"
            }
            if (currentBroker != newBroker) {
                currentBroker = newBroker
                reportBody.append(currentBroker)
                reportBody.append("\n")
                
            }
            reportBody.append("\t" + entry.get("Symbol"))
        ...
        // email the report
        if (opt.r) {
            File attachment = new File("broker-summary_" + today.toString() + ".xls")
