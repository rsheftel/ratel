...
/**
 * Copy of <code>quickfix.SLF4JLog</code> so that we can use a newer version of SLF4J and add some features to
...
public class Slf4jLog extends SLF4JLog {
    ...
    public void onEvent(String text) {
        ...
        } else if (text.contains("MsgSeqNum too high")) {
            // This has the session id then the message
            String msg = text;
            int colon = text.indexOf(':');
            if (colon != -1) {
                msg = text.substring(colon + 1);
            }
            pcs.firePropertyChange(new PropertyChangeEvent(this, MSG_SEQ_NUM, "", msg));
        }
    }
