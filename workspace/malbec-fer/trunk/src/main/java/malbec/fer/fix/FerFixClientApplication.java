...
public class FerFixClientApplication extends FixClientApplication implements IPropertyChangeSupport {
        ...
        try {
            ...
            if (isFill(execType) && orderStatus != OrdStatus.NEW) {
                ...
                if (OrdStatus.NEW == orderStatus) {
                    ...
                    // Apollo sends pending and then new
                ...
                } else if (OrdStatus.DONE_FOR_DAY == orderStatus) {
                    FerFixEvent ffe = new FerFixEvent(this, DoneForDay, message);
                    pcs.firePropertyChange(ffe);
                    fixLog.info("Fired event: OrderUpdate-DONE-FOR-DAY");
                } else {
    ...
    private String getRejectReason(Message fixMessage) {
        if (fixMessage.isSetField(Text.FIELD)) {
            try {
                ...
                String textMessage = getTextErrorMessage(fixMessage);
    ...
    protected String getTextErrorMessage(Message fixMessage) throws FieldNotFound {
        return fixMessage.getString(Text.FIELD);
    }
