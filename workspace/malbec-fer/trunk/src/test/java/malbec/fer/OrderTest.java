...
public class OrderTest extends AbstractBaseTest {
    ...
    private static final String LIMIT_ORDER_QUANTITY_STR = "100";
    private static final String LIMIT_PRICE_STR = "125.4";
    private static final String STOP_PRICE_STR = "145.9";
    ...
    public void testInsertAndUpdate() {
        ...
        // update the buy to cover flag
        orderCopy.setBuyToCover("Y");
        orderCopy.setOrderID(String.valueOf(System.currentTimeMillis()));
        OrderDao.getInstance().updateOrder(orderCopy);
        
        em = emf.createEntityManager();
        Order dbOrder = em.getReference(Order.class, orderCopy.getId());
        assertNotNull(dbOrder);
        assertEquals(dbOrder.getBuyToCover(), "Y");
        
        em.close();
        
        Order orderIdOrder = OrderDao.getInstance().findByOrderId(orderCopy.getOrderID(), orderCopy.getOrderDate(), orderCopy.getPlatform());
        assertNotNull(orderIdOrder);
