...
public class DateTimeUtil {
    ...
    /**
     * Create the <code>DateTime</code> for the day of week and time.
     * 
     * JodaTime week is Monday -> Sunday.
     * 
     * @param dayOfWeek
     * @param timeOfDay
     * @param beforeOrSame
    ...
    public static DateTime createDateTime(String dayOfWeek, LocalTime timeOfDay, boolean beforeOrSame) {
        ...
        LocalDate.Property dp = today.property(DateTimeFieldType.dayOfWeek());
        LocalDate createdDate = dp.setCopy(dayOfWeek);

        // Check if we are setting the start of the schedule and the day is Sunday
        boolean isSaturday = today.dayOfWeek().get() == DateTimeConstants.SATURDAY;
        if (beforeOrSame && "Sunday".equalsIgnoreCase(dayOfWeek) && createdDate.isAfter(today) && !isSaturday) {
            createdDate = createdDate.minusWeeks(1);
        } else if (!beforeOrSame && createdDate.isBefore(today)) {
            createdDate = createdDate.plusWeeks(1);
        }
    ...
    public static DateTime[] determineWeeklyRange(String startDay, String startTime, String endDay,
        ...
        DateTime startDateTime = combineDateTime(defaultRange[0].toLocalDate(), startTimeGuess.toDateTimeToday(), defaultRange[0].getZone());
        DateTime endDateTime = combineDateTime(defaultRange[1].toLocalDate(), endTimeGuess.toDateTimeToday(), defaultRange[1].getZone());
