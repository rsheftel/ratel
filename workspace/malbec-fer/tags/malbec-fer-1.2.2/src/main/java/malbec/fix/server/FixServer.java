...
import malbec.fix.FixClientMBean;
 ...
 * 
 */
public class FixServer implements FixClientMBean {
    ...
    private FixServerApplication application;
    ...
    static {
        TaskService.getInstance().addExecutor("FixClient",
                Executors.newSingleThreadScheduledExecutor(new NamedThreadFactory("FixClientSchedule")));
    }

    public FixServer(String sessionName) {
        this.sessionName = sessionName;
        this.loggerConfig = this.sessionName;
        this.log = LoggerFactory.getLogger(loggerConfig);
    }

    public FixServer(String sessionName, Properties config, EmailSettings emailSettings) {
        this(sessionName, new FixServerApplication(emailSettings), config, emailSettings);
    }

    public FixServer(String sessionName, FixServerApplication clientApp, Properties config, EmailSettings emailSettings) {
        this(sessionName);
    ...
    private void setPassword(FixServerApplication clientApp, Properties config) {
    ...
    private void createAcceptorSafe() {
        try {
            createAcceptor();
        } catch (InvalidConfigurationException e) {
    ...
    private void createAcceptor() throws InvalidConfigurationException {

        if (config.containsKey(SessionFactory.SETTING_CONNECTION_TYPE)
                && !"acceptor".equals(config.getProperty(SessionFactory.SETTING_CONNECTION_TYPE))) {
            throw new InvalidConfigurationException("Connector type not set to 'acceptor' - "
        ...
        try {
            acceptor = new SocketAcceptor(application, msgStoreFactory, qfjSettings, logFactory,
                    messageFactory);
        } catch (ConfigError e) {
        ...
        ArrayList<SessionID> allSessions = acceptor.getSessions();
    ...
    public void start() {
        if (monitorFuture == null || monitorFuture.isCancelled()) {
            ...
            monitorFuture = executor.scheduleAtFixedRate(new ClientSessionMonitor(schedule, this, log), 0, 1,
                    TimeUnit.SECONDS);
            StringBuilder sb = new StringBuilder(256);
            DateTime startTime = getClientStartTime();
            DateTime endTime = getClientEndTime();

            sb.append("Client starts on ").append(startTime.property(DateTimeFieldType.dayOfWeek()).getAsText());
    ...
    public void setConfiguration(Properties config) throws InvalidConfigurationException {
        ...
        setConfiguration(new FixServerApplication(emailSettings), config);
     ...
     * @Deprecated
     */
    void setConfiguration(FixServerApplication app, Properties config) throws InvalidConfigurationException {
        ...
        LogFactory logFactory = new Slf4jLogFactory(qfjSettings);
        try {
            acceptor = new SocketAcceptor(app, msgStoreFactory, qfjSettings, logFactory, messageFactory);
        } catch (ConfigError e) {
            throw new InvalidConfigurationException("Unable to create initiator", e);
        }
    ...
    private void configureSchedule(ClientSchedule schedule, Properties config) throws InvalidConfigurationException {
        ...
        if (clientStartTime == null || clientEndTime == null) {
            ...
            if (clientStartTime == null || clientEndTime == null) {
                throw new InvalidConfigurationException("Unable to configure client schedule, missing start/end time");
            }
        }
        ...
        schedule.setStartDayTime(clientStartDay, clientStartTime);
        schedule.setEndDayTime(clientEndDay, clientEndTime);
    }
    ...
    private static final class ClientSessionMonitor implements Runnable {
        ...
        private final FixServer fixClient;
        private final Logger log;

        private boolean initiatorStarted;

        ClientSessionMonitor(ClientSchedule cs, FixServer fixClient, Logger log) {
            this.schedule = cs;
        ...
        public void run() {
            ...
            if (schedule.isWithinSchedule() && !initiatorStarted) {
                try {
                    ...
                    if (fixClient.getAcceptor() == null) {
                        fixClient.createAcceptorSafe();
                    }
                    fixClient.getAcceptor().start();
    ...
    public boolean sendFixMessage(Message fixMessage) {
