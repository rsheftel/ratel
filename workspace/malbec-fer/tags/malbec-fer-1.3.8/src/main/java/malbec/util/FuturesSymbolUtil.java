...
public class FuturesSymbolUtil {
    ...
    public static String extractSymbolRoot(String futuresSymbol) {
        ...
        else if (futuresSymbol.matches("[A-Za-z|\\d][A-Za-z ][A-Za-z]\\d[:][A-Za-z][A-Za-z]")) {
            int colonPos = futuresSymbol.lastIndexOf(':');
            if (colonPos > 1) {
                return removeMonthAndYear(futuresSymbol.substring(0, colonPos));
            } else {
                log.error("Identified as RIC with colon (:) but unable to extract root: " + futuresSymbol);
                return futuresSymbol;
            }
        }
    ...
    public static String bloombergToMaturityMonthYear(String symbol) {
        ...
        if (colonPos > -1) {
            symbol = symbol.substring(0, colonPos);
        }
    ...
    public static String extractMaturityMonthFromSymbol(String symbol) {
        ...
        String[] colonParts = symbol.split(":");
        symbol = colonParts[0];
