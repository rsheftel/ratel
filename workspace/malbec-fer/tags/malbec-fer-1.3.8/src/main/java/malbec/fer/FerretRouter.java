...
import malbec.fer.jms.ITransportInterruptedListener;
...
public class FerretRouter implements FerretRouterMBean, PropertyChangeListener, IMessageProcessor,
    ITransportInterruptedListener {

    final private Logger log = LoggerFactory.getLogger(getClass());

    private final List<FerretState> STATES_THAT_ACCEPT_ORDERS = new ArrayList<FerretState>();

    private final PositionCache positionCache = PositionCache.getInstance();

    private String positionBrokerUrl;

    private String equityPrimeBroker;
    ...
    public void addPositionBroker(String brokerUrl, String primeBroker) {

    }

    private static class TransportDelegate implements PropertyChangeListener {

        private ITransportInterruptedListener listener;

        public TransportDelegate(ITransportInterruptedListener listener) {
        ...
        public void propertyChange(PropertyChangeEvent evt) {
            ...
            if ("Interrupted".equalsIgnoreCase(event)) {
                listener.transportInterrupted();
            } else if ("Resumed".equalsIgnoreCase(event)) {
    ...
    public void addJmsConnection(JmsServerSessionApp jsa) {
        ...
        TransportDelegate td = new TransportDelegate(this);

        jsa.addPropertyChangeListener("Interrupted", td);
        jsa.addPropertyChangeListener("Resumed", td);
    }
    ...
    public void start() throws InvalidConfigurationException {
        ...
        if (positionBrokerUrl != null) {
            try {
                Connection connection = createStartedBrokerConnection();
                positionCache.setBrokerConnection(connection);
                positionCache.setPrimeBroker(equityPrimeBroker);

                positionCache.start();
                log.info("Started Position Monitor using: " + positionBrokerUrl + " and prime broker: "
                    + equityPrimeBroker);
            } catch (JMSException e) {
                log.error("Unable to start Position Listener", e);
            }
        }
        // This is misleading, but we only every have one client.
        String brokerUrl = null;
        for (AbstractJmsApp jmsApp : jmsClients) {
            brokerUrl = jmsApp.getBrokerUrl();
            jmsApp.start();
        ...
        if (brokerUrl != null) {
            try {
                startFerretStateMonitor(brokerUrl);
            } catch (JMSException e) {
                log.error("Unable to start FerretStateMonitor", e);
            }
        }
    }

    private Connection createStartedBrokerConnection() throws JMSException {
        ActiveMQConnectionFactory factory = new ActiveMQConnectionFactory(positionBrokerUrl);
        Connection connection = factory.createConnection();
        connection.start();
        return connection;
    }

    private void startFerretStateMonitor(String brokerUrl) throws JMSException {
        ActiveMQConnectionFactory factory = new ActiveMQConnectionFactory(brokerUrl);
        Connection brokerConnection = factory.createConnection();
        brokerConnection.start();

        // Create the state listener
        ferretStateHandler = new DestinationHandler(brokerConnection, "FER.State", true);
        ferretStateHandler.setTextMessageProcessor(new DefaultTextMessageProcessor() {
            ...
            protected void onTextMessage(TextMessage textMessage, Map<String, String> mapMessage) {
                ...
                log.info("Received Ferret state message: " + ferretState);
                if (ferretState != null) {
                    String oldState = currentStateString();
                    if (!ferretState.equalsIgnoreCase(oldState)) {
                        log.error("Ferret State topic has wrong Ferret state value!  Should be: " + oldState
                            + " but is: " + ferretState);

                        if (emailAlertManager == null) {
                            emailAlertManager = new EmailAlertManager(emailSettings);
                        }

                        emailAlertManager.send("ExternalStateChange",
                            "External program changed Ferret state",
                            "External application published incorrect Ferret state: " + ferretState
                                + ", should be: " + oldState);
    ...
    protected synchronized void ferretScheduleTimer() {
        try {
            ...
            if (lastCheckedFerretState != fs) {
                log.info("Ferret state changed from: " + lastCheckedFerretState + " to: " + fs);
                lastCheckedFerretState = fs;

                if (publishFerretState(fs)) {
                    log.info("Ferret state is " + lastCheckedFerretState);
                } else {
                    log.warn("Unable to publish Ferret state of " + fs);
                }
            }
        } catch (Exception e) {
            log.error("Problem with schedule thread", e);
            lastCheckedFerretState = null;
        }
    }
    ...
    public void stop() {
        ...
        try {
            ...
            log.error("Exception when stopping position cache", e);
        }
    }
    ...
    Map<String, String> processOrderRequest(IMessageProcessListener mpl, Map<String, String> message,
        ...
        if (!acceptingOrders(ferretState)) {
            ...
            log.warn("Rejecting order - Ferret state: " + ferretState+"\n" + message);
            return message;
        }
    ...
    private void propertyChange(FerFixEvent ferEvent) {
        try {
            ...
            } else if (FixEventType.DoneForDay == eventType) {
                handleEventDoneForDay(ferEvent, dao);
            } else {
    ...
    private void handleEventDoneForDay(FerFixEvent ferEvent, OrderDao dao) {
        String clientOrderID = ferEvent.getClientOrderId();

        Order uo = dao.updateOrder(clientOrderID, DoneForDay);
        sendReponse(uo, DoneForDay);
    }
    ...
    private void handleEventReplaced(FerFixEvent ferEvent, OrderDao dao) {
        ...
        if (originalClientOrderId != null) {
            ...
            String senderCompId = getStringField(fixMessage, SenderCompID.FIELD);
            if ("REDI".equalsIgnoreCase(senderCompId)) {
                Order uo = dao.updateOrder(clientOrderID, PendingCancel);
                sendReponse(uo, PendingCancel);
            } else {
                // If tag 50 is set, we are a live order (TradingScreen). If
                // tag 50 ends in ':RE' it is a released registered order
                // This is logic for TradingScreen, but is probably more correct
                // than the Redi logic
                Order uo = dao.updateOrder(clientOrderID, Replaced);
                sendReponse(uo, Replaced);
            }
        }
    }
    ...
    private void handleEventFill(FerFixEvent ferEvent, OrderDao dao) {
        try {
            ...
            if (fill.getLeavesQuantity() > 0d) {
                ...
                if (uo instanceof CancelRequest) {
                    sendEventError(ferEvent, "Updated CancelRequest instead of Order");
                }
            } else {
    ...
    private void handleEventReject(FerFixEvent ferEvent, OrderDao dao) {
        ...
        // TradingScreen sends orderId on rejected orders. Redi might as well
        if (orderId != null && orderId.trim().length() != 0) {
            dao.updateOrderWithOrderId(clientOrderID, PlatformRejected, orderId);
        }
    ...
    private String getStringField(Message fixMessage, int field) {
        try {
            return fixMessage.getHeader().getString(field);
        } catch (FieldNotFound e) {
            return null;
        }
    }
    ...
    private long insertFixFill(FixFill fill) {
        try {
            ...
            log.error("Unable to persist fill", e);
            return -1;
        }
    }
    ...
    private long insertExecutionReport(Message fixMessage) {
        ...
        try {
            ...
            log.error("Unable to persist execution: " + fixMessage, e);
            return -1;
        }
    ...
    private boolean publishFerretState(FerretState fs) {
        boolean published = false;
        for (JmsServerSessionApp jmsApp : jmsClients) {
            published = jmsApp.publishStatus(fs);
        }

        return published;
    }

    public synchronized FerretState currentState() {
    ...
    public void transportInterrupted() {
        ...
        log.error("JMS Transport has been interrupted");
    }
    ...
    public void transportResumed() {
        synchronized (this) {
            if (interrupted) {
                log.info("JMS Transport has resumed, publishing Ferret state");
                publishFerretState(currentState());
                interrupted = false;
            }
        }
    ...
    /**
     * @return the positionBrokerUrl
     */
    public String getPositionBrokerUrl() {
        return positionBrokerUrl;
    }

    /**
     * @param positionBrokerUrl
     *            the positionBrokerUrl to set
     */
    public void setPositionBrokerUrl(String positionBrokerUrl) {
        this.positionBrokerUrl = positionBrokerUrl;
    }

    /**
     * @return the equityPrimeBroker
     */
    public String getEquityPrimeBroker() {
        return equityPrimeBroker;
    }

    /**
     * @param equityPrimeBroker
     *            the equityPrimeBroker to set
     */
    public void setEquityPrimeBroker(String equityPrimeBroker) {
        this.equityPrimeBroker = equityPrimeBroker;
    }
