...
public class OrderProcessor extends AbstractOrderRequestProcessor {
    ...
    protected ITransportableOrder createTransportableOrder(Order order,
        ...
        if (activeSession) {
            ...
            // We need to round all equity trades to lots of 100.  Do this here
            
            if (order.isEquity()) {
                BigDecimal equityLot = MathUtil.truncate(order.getQuantity(), 100);
                if (BigDecimal.ZERO.compareTo(equityLot) == 0) {
                    errors.add("Equity size truncated to 0, order rejected");
                }
                order.setQuantity(equityLot);
            }
