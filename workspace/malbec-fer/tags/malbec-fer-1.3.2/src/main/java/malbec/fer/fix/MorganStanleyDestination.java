...
/**
 * TODO Implement logic for Morgan Stanley
 */
public class MorganStanleyDestination extends FixDestination {

    public MorganStanleyDestination(String name, Properties config, EmailSettings emailSettings, DatabaseMapper dbm) {
    ...
    protected void populateCommonFields(Order order, Message fixMessage, List<String> conversionErrors) {
        ...
        if (! "PP".equalsIgnoreCase(order.getExchange())) {
            conversionErrors.add("Wrong exchange value for Passport (PP): " + order.getExchange());
        }
