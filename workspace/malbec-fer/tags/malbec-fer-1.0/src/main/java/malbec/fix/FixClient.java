 ...
 * the <code>Application</code> and the <code>SessionStateListener</code> to arrive in random order. For
 * this reason (and others) each <code>FixClientSession</code> will have it's own initiator that is single
 * threaded.
 * 
 */
public class FixClient implements FixClientMBean {
    ...
    public FixClient(String sessionName) {
        this.sessionName = sessionName;
        this.loggerConfig = this.sessionName;
        this.log = LoggerFactory.getLogger(loggerConfig);
    }

    public FixClient(String sessionName, FerFixClientApplication clientApp, Properties config) {
        this(sessionName);
     ...
     * @Deprecated
     */
    void setConfiguration(Application app, Properties config) throws InvalidConfigurationException {
        ...
        LogFactory logFactory = new Slf4jLogFactory(qfjSettings);
        try {
            initiator = new SocketInitiator(app, msgStoreFactory, qfjSettings, logFactory, messageFactory);
        } catch (ConfigError e) {
            throw new InvalidConfigurationException("Unable to create initiator", e);
        }
    ...
    public boolean sendOrder(NewOrderSingle nos) {
        return sendOrder((Message) nos);
    }
