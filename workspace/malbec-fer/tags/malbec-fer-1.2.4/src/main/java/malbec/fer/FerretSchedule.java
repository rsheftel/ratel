...
import static malbec.util.DateTimeUtil.nowMillis;
...
public class FerretSchedule {
    ...
    private WeeklyRange buildActiveRange(String activeRangeStart, String activeRangeEnd) {
        DateTime activeStart = createDateTime("Monday", "04:30:00", true);
        if (activeRangeStart != null) {
            try {
                String[] rangeStart = activeRangeStart.split(" ");
                createDateTime(rangeStart[0], rangeStart[1], true);
            } catch (Exception e) {
                log.warn("Unable to parse activeRangeStart: " + activeRangeStart);
            }
        }

        DateTime activeEnd = createDateTime("Friday", "16:30:00", false);
        if (activeRangeEnd != null) {
            try {
                String[] rangeEnd = activeRangeEnd.split(" ");
                activeEnd = createDateTime(rangeEnd[0], rangeEnd[1], false);
            } catch (Exception e) {
                log.warn("Unable to parse activeRangeEnd: " + activeRangeEnd);
            }
        }
        return new WeeklyRange(activeStart, activeEnd);
    }
    ...
    public boolean advanceActiveSchedule(DateTime now) {
