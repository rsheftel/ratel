...
public class JmsSession implements IConnectable, IJmsSession {
    ...
    public void start() {
        if (monitorFuture == null || monitorFuture.isCancelled()) {
            ScheduledExecutorService executor = (ScheduledExecutorService) TaskService.getInstance().getExecutor("JmsConnection");
    ...
    private synchronized MessageConsumer createConsumerWithSelector(final String selector) throws JMSException {
