...
public class JmsServerSessionApp extends AbstractJmsApp implements MessageListener, IMessageProcessListener,
    ...
    public void start() {
        try {
            ...
            jmsSession.connect((TransportListener) this);
            super.start();
    ...
    public int broadcastStatus(String userOrderId, LocalDate orderDate, Map<String, String> message,
        ...
        if (destinations == null || destinations.size() == 0) {
            log.warn("No JMS destinations found for clientOrderId: " + userOrderId);
            // return 0;
    ...
    public boolean publishStatus(FerretState fs) {
        try {
            JmsServerSession jmsSession = getJmsSession();
            if (jmsSession.isConnected()) {
                Topic topic = jmsSession.getTopicFor("FER.State");
    ...
    public void onException(IOException error) {
        pcs.firePropertyChange("Exception", null, error);
    }

    @Override
    public void transportInterupted() {
        pcs.firePropertyChange("Interrupted", null, true);
    }

    @Override
    public void transportResumed() {
        pcs.firePropertyChange("Resumed", null, true);
    }
