...
//    public static Executor instance;
    ...
    public static synchronized void setRun(boolean b) {
        run = b;
    }
    
    public static synchronized boolean getRun() {
        return run;
    }
    ...
    public static void main(String args[]) throws Exception {
        try {
            System.out.println("Starting Local Executor");
            
            InputStream inputStream = getSettingsInputStream(args);
            SessionSettings settings = new SessionSettings(inputStream);
            Properties props = settings.getDefaultProperties();
            
            // this is for when we are testing as an application, instead of
            // a unit test
            //ResetOnLogon
            props.setProperty("ResetOnLogon", System.getProperty("ResetOnLogon", "Y"));
            inputStream.close();

            Executor executor = new Executor(settings);
            // We need access to this so we can stop it when running unit tests
