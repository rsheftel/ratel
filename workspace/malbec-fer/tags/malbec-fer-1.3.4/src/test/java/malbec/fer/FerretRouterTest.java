 ...
 * 
 */
public class FerretRouterTest extends AbstractFixTest {
    ...
    public void testRouteOrder() throws Exception {
        ...
        assertNotNull(MessageUtil.getOrderDate(response));
...
//        assertFalse(response.containsKey("ERROR_1"), "Response has errors" + response.get("ERROR_1"));
        ...
        // send a market on close order -- this should work (normal market orders do not)
        Order moc = jmsHarness.createMarketOrder("FixServer");
        moc.setStrategy("TESTE");
        moc.setTimeInForce("CLOSE");
        moc.setOrderType("MARKET");

        String mocId = jca.sendOrder(moc);
        assertNotNull(mocId, "Failed to send order");

        jmsHarness.waitForResponse(jca, mocId);
        Map<String, String> mocResponse = jca.getResponseFor(mocId);
        assertNotNull(mocResponse, "Did not receive response for JMSMessageID: " + mocId);
        assertFalse(mocResponse.containsKey("ERROR_1"), "Thre is an error with order");
    ...
    @Test(groups = { "unittest" })
    public void testOrderForMarketTicker() throws Exception {
        System.out.println("Starting 'testRouteOrder'");
        ...
        // send a good order (limit) with market ticker
        Order order = OrderTest.createLimitOrder("FixServer");
        order.setSymbol("TY.1C");
        order.setSecurityType("Futures");
        order.setStrategy("TESTF");
        String jmsMessageID = jca.sendOrder(order);
        assertNotNull(jmsMessageID, "Failed to send order");
        ...
        String error1 = response.get("ERROR_1");
        assertFalse(response.containsKey("ERROR_1"), "Response has errors ("+error1+")");
        assertEquals(response.get("SYMBOL"), "TYM9");
    }
