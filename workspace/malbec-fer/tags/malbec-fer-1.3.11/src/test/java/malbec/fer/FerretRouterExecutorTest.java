...
public class FerretRouterExecutorTest extends AbstractBaseTest {
    ...
    static {
        ...
        TaskService.getInstance().createAndAddSingleThreadScheduled("ExecutorMainThread");
    }
...
//    IDatabaseMapper dbm = createDatabaseMapper();
    ...
    public void startQuickFix() throws Exception {
        ...
        ScheduledExecutorService executor = TaskService.getInstance().getExecutor("ExecutorMainThread");
        // We store a public static reference to the Executor so we can shut it down
        future = executor.submit(new Runnable() {
            @Override
            public void run() {
                try {
                    System.out.println("Starting executor on thread: " + Thread.currentThread().getName());
                    Executor.setRun(true);
    ...
    public void testCancelReplaceOrder44() throws Exception {
        ...
        String errorMessage = mpl.getErrorMessageFor(crr.getUserOrderId());
        if (!errorMessage.contains("Order of status Filled")) {
