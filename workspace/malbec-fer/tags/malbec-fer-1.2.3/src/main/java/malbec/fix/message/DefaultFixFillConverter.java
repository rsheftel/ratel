...
public class DefaultFixFillConverter implements IFixFillConverter {
    ...
    protected void extractFuturesSymbolDetails(FixFill fill, Message er) throws FieldNotFound {
        ...
        double multiplier = getDatabaseMapper().lookupFuturesInboundPriceMultiplier(getPlatform(), futuresRootSymbol);
        fill.setPriceMultiplier(BigDecimal.valueOf(multiplier));
         
        String bloombergSymbol = getDatabaseMapper().mapPlatformRootToBloombergSymbol(getPlatform(), futuresRootSymbol, fill.getMaturityMonth());
        fill.setSecurityIdSource("A"); // Bloomberg
        fill.setSecurityId(bloombergSymbol);

        fill.setBloombergProductCode(getDatabaseMapper().lookupFuturesProductCode(getPlatform(), futuresRootSymbol));
    }
