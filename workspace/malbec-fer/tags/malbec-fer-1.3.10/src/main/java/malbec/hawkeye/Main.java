...
public class Main extends AbstractJavaServiceWrapperMain {
    ...
    protected boolean validateStatusMessageMain(Map<String, String> mapMessage) {
        ...
        if (!shouldFerretBeSending() && "SENT".equalsIgnoreCase(status)) {
        ...
        if ("Invalid".equalsIgnoreCase(status) || "FerretRejected".equalsIgnoreCase(status)) {
            ...
            log.warn("Invalid/rejected order received by the Ferret: " + mapMessage);
        }
        return true;
    }
        ...
        if (!ordersMatch(tomahawkOrders, ferretOrders)) {
            ...
            if (extraTomahawkCount > 0) {
                if (!hadMoreTomahawkOrders) {
                    ...
                    log.warn("First check, differences found");
                } else {
