...
//        envProps.list(System.out);
        ...
        LocalDate tradingDay = mostLikelyTradingDate();
        ...
        logger.info("Set trading day to " + tradingDay);
        // start MBeans
        ...
        app.publishExcelBatchPositions();
        app.publishSecurityRootPositions();
        app.publishPrimeBrokerBatchPositions();
        app.publishDefaultPositionBatchPositions();
        app.publishDefaultSymbolYelloKeyPositionBatchPositions();
        ...
        // set the scheduler to pick up the new batch file after it is down loaded at 6:10 PM
        // There are issues with DST change that not all servers have the patch so in March
        // the file may be down loaded after we read it.
...
//        RtfMessage overRideRequest = new DefaultRtfMessage(new RtfHeader(RtfCommand.Override, new LocalDate(2008, 3, 11), 1), null);
        ...
        if (app.isConnected(realtimeSession)) {
            ...
            logger.warn("Realtime session not connected, not sending over-ride request");
        }
    ...
    private static LocalDate mostLikelyTradingDate() {
        LocalDate tradingDate = new LocalDate();
        
        while (isWeekend(tradingDate)) {
            tradingDate = tradingDate.plusDays(1);
        }
    ...
    public static void rescheduleBatchReload(RealtimeApplication rtApp, LocalTime reloadTime) throws SchedulerException {

        try {
            Scheduler sched = DirectSchedulerFactory.getInstance().getScheduler();

            // Create a unique name for the job
            JobDetail jobDetail = new JobDetail("myJob"+ System.currentTimeMillis(), null, ReloadBatchFileJob.class);
            jobDetail.getJobDataMap().put("application", rtApp);
            
            SimpleTrigger trigger = new SimpleTrigger("Retry EOD", "NoGroup", reloadTime.toDateTimeToday().toDate());
