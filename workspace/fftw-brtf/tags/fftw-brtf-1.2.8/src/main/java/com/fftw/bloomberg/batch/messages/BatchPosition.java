...
public class BatchPosition implements PositionRecord, PropertyChangeListener {
    ...
    public BigDecimal getOnlineOpenPosition() {
        ...
        if (("FUTURE".equalsIgnoreCase(securityType2) && !(productCode == BBProductCode.Index))
            ...
            && (currency != baseCurrency  && baseCurrency == Currency.getInstance("USD"))) {
    ...
    public Position getPosition() {
        ...
        int numberOfBuys = 0;
        int numberOfSells = 0;
        BigDecimal currentPosition = normalizeShares(fullCurrentNetPositionWithoutComma);
        ...
        if (onlinePosition != null) {
            ...
            currentPosition = onlinePosition.getNormCurrentPosition();
        }

        int intradayPosition = numberOfBuys - numberOfSells;

        PositionType positionType = currentPosition.intValue() > 0 ? PositionType.Long : PositionType.Short;

        Position position = new Position(getSecurity(), getAccount(), getLevel1TagName(), getLevel2TagName(),
            getLevel3TagName(), getLevel4TagName(), getPrimeBroker(), getOnlineOpenPosition(),
