...
public class BatchPosition implements PositionRecord, Aggregatable<BatchPosition> {
    ...
    private final static BigDecimal MULTIPLIER = BigDecimal.valueOf(1000);
    
    /**
     * This is calculated since Bloomberg is inconsistent
     */
    private BigDecimal onlineCurrentPosition; 
    ...
    public BigDecimal getOnlineCurrentPosition() {
        // This is called many times, make it as fast as possible
        if (onlineCurrentPosition != null) {
            return onlineCurrentPosition;
        }
        
        if ("FUTURE".equalsIgnoreCase(securityType2)) {
            onlineCurrentPosition = currentLongPosition.divide(contractSize).multiply(MULTIPLIER);
        } else {
            onlineCurrentPosition = fullCurrentNetPositionWithoutComma;
        }
...
//        return positionName;
    ...
    public String toTextMessage(String delimiter) {
        ...
        sb.append(delimiter).append("openPositionKey=").append(getOnlineCurrentPosition().longValue());
