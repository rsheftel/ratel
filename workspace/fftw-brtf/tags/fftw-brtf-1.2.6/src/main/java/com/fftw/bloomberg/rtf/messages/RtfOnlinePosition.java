...
public class RtfOnlinePosition implements RtfMessageTradeBody, PositionRecord {
    ...
    // logic to deal with Old excel logic
    private boolean useExcelKey;
    ...
    public String getSecurityId() {
        if (useExcelKey && productCode == BBProductCode.Currency && bloombergId != null) {
            // We might need to concatenate 
            String bid = bloombergId.trim();
            if (bid.length() > 0 && (!bid.startsWith(securityId) || bid.contains("/"))) {
                return securityId + "_" + bloombergId;
            }
        }
        return securityId;
    }
