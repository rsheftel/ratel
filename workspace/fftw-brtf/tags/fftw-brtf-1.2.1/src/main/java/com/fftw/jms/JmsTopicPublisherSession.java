    ...
    public void publishHeartBeat() {
        try {
            ...
            for (MessageProducer publisher : publisherMap.values()) {
                if (publisher.getDestination().toString().toUpperCase().contains("AIM")) {
                    continue;
                }
...
// reconnect
    ...
    public void publishToTopicLastImage(RtfOnlinePosition message) {
        //String msgStr = app.createPositionMessage(message);
        String msgStr = message.toReconString();
        StringBuilder sb = new StringBuilder(msgStr.length() + 50);
        sb.append(msgStr).append("|").append("TIMESTAMP=").append(DateTimeUtils.formatNow());
