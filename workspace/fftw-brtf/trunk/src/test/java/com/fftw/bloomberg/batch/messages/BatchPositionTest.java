...
//     "/GissingAf200DEV-2008-03-11-v1.txt",
    ...
    private String[] BATCHPOSITIONFILES = {
        "/BatchPositionTest-2009-07-21.txt"
    };
...
//        assertEqualsBD(position.getCurrentPosition(), bp.getCurrentShortPosition());
    ...
    @Test(groups = { "unittest" })
    public void testBatchPositionDisplay() {
        // for now, read in a sample file and parse it.
        int lineCount = 0;
        String line = "";
        String globlalFilenameStr = "";
        ...
        try {
            for (String filename : BATCHPOSITIONFILES) {
                System.out.println("Processing file: " + filename);
                globlalFilenameStr = filename;
                LocalDate fileDate = extractDateFromFilename(filename);
                URL location = BatchPositionTest.class.getResource(globlalFilenameStr);
                ...
                while ((line = batchPositionReader.readLine()) != null) {
                    ...
                    if ("QMF".equals(bp.getAccount())) {
                        // For now we have only one test
                        if ("EUR/USD2".equals(bp.getSecurityId())) {
                            if (bp.getCurrentPosition() != null) { 
                                assertEqualsBD(bp.getCurrentPosition(), "-330077.03");
                            }
                            assertEqualsBD(bp.getOnlineOpenPosition(), "-330077.03");
                            assertEqualsBD(position.getCurrentPosition(), "-330077.03");
                            assertEqualsBD(position.getOpenPosition(), "-330077.03");
