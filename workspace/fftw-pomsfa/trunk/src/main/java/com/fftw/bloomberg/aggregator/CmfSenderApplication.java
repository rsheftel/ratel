...
public class CmfSenderApplication implements CmfApplication
{
    ...
    private void updateSentTrade (CmfMessage cmfMessage)
    {
        ...
        if (item == null) {
            ...
            log.error("Received update to trade record, but record not in cache." + cmfMessage);
        }
    ...
    {
        // We need to slow down to 5 messages per second.
        ...
        if (tradeSequenceNumber != -1)
        {
            ...
            while (!dstSession.isLoggedOn()) {
                // we are probably starting up, wait a few seconds
                try
                {
                    Thread.sleep(5000);
                }
        ...
        // ensure we take 200 milliseconds per message
        if (diff < 200) {
            try
            {
                long sleepLength = 200 - diff;
                log.info("Throttle: sleeping for "+ sleepLength +" milliseconds");
