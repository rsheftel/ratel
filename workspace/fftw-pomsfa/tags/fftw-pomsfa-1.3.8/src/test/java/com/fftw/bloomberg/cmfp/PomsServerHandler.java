...
public class PomsServerHandler extends IoHandlerAdapter
{
    ...
    public PomsServerHandler(String pricingNumber, String specVersion)
    {
    ...
    public void messageReceived (IoSession session, Object msg) throws Exception
    {
        ...
        if (CmfConstants.HEARTBEAT_ACK.equals(header.getMessageType())) {
            ...
            CmfMessage tradeReceipt = createTradeReceipt(pricingNumber, specVersion, message.getTradeRecord());
            sendMessage(session, tradeReceipt);
            CmfMessage tradeAccept = createTradeAccept(pricingNumber, specVersion, message.getTradeRecord());
            tradeAccept.getTradeRecord().setReturnCode(0);
            tradeAccept.getTradeRecord().setMessageType(CmfConstants.ACCEPT_REJECT);
            tradeAccept.getTradeRecord().setTicketNumber(ticketNumber++);
            sendMessage(session, tradeAccept);
        } else {
            System.err.println("Received messageType:"+header.getMessageType());
        }
    }

    private CmfMessage createTradeAccept (String priceNumber, String version, CmfTradeRecord tradeRecord)
    {
    ...
    private CmfMessage createTradeReceipt (String priceNumber, String version, CmfTradeRecord tradeRecord)
    {
