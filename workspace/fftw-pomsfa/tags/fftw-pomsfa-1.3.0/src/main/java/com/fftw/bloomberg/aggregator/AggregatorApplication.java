...
public class AggregatorApplication extends MessageCracker implements Application
{
                ...
                if (cmfMessage != null)
                {
                    try
                    {
                        ...
                        if (useRemote)
                        {
                            ...
                            // client.getRemoteDispatcher("tcp://nysrv31:60504");
                            dispatcher.sendRemote(destinationUri, cmfMessage, null);
                        }
                        else
                        {
                            // Send as a string so we don't have to deal with
                            // serialiation
                            // client.send(destinationUri,
                            // cmfMessage.getWireMessage(), null);
    ...
    public void onMessage (Allocation message, SessionID sessionID)
    {
        ...
        // account
        // to allocate to, but we already created the BB record by then and will
        // not easily be able to match
    }
        ...
        if (message.isSetField(Text.FIELD))
        {
                ...
                || txt.contains("SEQUENCE NUMBER: Seq"))
            {
                try
                {
                    ...
                    log.warn("NextSender sequence number should be " + expectedSeq + " for sessionID:"
                        + sessionID);
