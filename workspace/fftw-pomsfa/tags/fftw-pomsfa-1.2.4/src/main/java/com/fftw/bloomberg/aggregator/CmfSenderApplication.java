...
public class CmfSenderApplication implements CmfApplication
{
    ...
    private static class ResendTimerTask extends TimerTask
    {
        ...
        public void run ()
        {
            application.log.info("Number of confirmations in queue="+ application.getSentTradeCount());
...
//                Date tmpDate = new Date(sentMillies + application.retryTimeout);
//                Date nowDate = new Date(now);
                ...
                if (sentTime.getTime() + application.retryTimeout < now)
                {
                    ...
                    application.log.info("Timer fired, waited: " + application.retryTimeout
                        + " giving up.");
                    application.mailer.emailErrorMessage("Exceeded retry logic - " + sentTime, cacheMessage
                        .getTradeRecord(), true);
...
//            if (tradeRecord.getSecurityIdFlag() == 99 && tradeRecord.getProductCode() == 10
//                && tradeRecord.getReturnCode() == 5)
//            {
//                // BB does not handle futures currency - ignore the error.
//                log.info("Futures currency failure:" + tradeRecord.getSecurityId());
//                return;
//            }
