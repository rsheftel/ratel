...
public class FixedWidthFormatter
{
     ...
     * @return
     */
    public static String formatNumber (double value, int wholeLength, int decimals)
    {
        // Set the significant numbers to the entire size
        MathContext mc = new MathContext(wholeLength + decimals);
        BigDecimal bd = new BigDecimal(value, mc);
        
        StringBuilder sb = new StringBuilder(10);
        sb.append("%0").append(wholeLength + decimals).append("d");

        bd = bd.movePointRight(decimals);
        
        return String.format(sb.toString(), bd.longValue()).substring(0, wholeLength + decimals);
    }

    public static String formatNumber(BigDecimal value, int wholeLength, int decimals)
    {
        // Set the significant numbers to the entire size
        MathContext mc = new MathContext(wholeLength + decimals);
