...
public class RediConversionStrategy extends AbstractConversionStrategy
{
    ...
    protected String populateFuturesSymbol (Message message, CmfTradeRecord tradeRecord)
        throws FieldNotFound
    {
        Symbol fullBbSymbol = extractSymbol(message);
        String futuresSymbol = fullBbSymbol.getValue();
        int lastSpace = futuresSymbol.lastIndexOf(' ');
        
        if (lastSpace > 1) { // make sure we don't mess up 'P U9'
            futuresSymbol = futuresSymbol.substring(0, lastSpace).trim();
        }
        tradeRecord.setSecurityId(futuresSymbol);
        String bbRoot = FuturesSymbolUtil.extractSymbolRoot(futuresSymbol);
