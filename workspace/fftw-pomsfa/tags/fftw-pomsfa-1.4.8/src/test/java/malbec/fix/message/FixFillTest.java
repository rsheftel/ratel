...
public class FixFillTest extends AbstractFixTest
{

    @BeforeClass(groups =
    {
        "unittest", "EMSXDropTest"
    })
    public void setup ()
    {
        try
    ...
    public void convertFix42Equity ()
    {
        ...
        // er.set(new SecurityType(SecurityType.EQUITY)); // There is no EQUITY,
        // if tag is missing == equity
        ...
        er.set(new ExecTransType(ExecTransType.NEW)); // CANCEL = '1'; CORRECT
                                                        // = '2'; STATUS = '3'
        er.set(new LastPx(9.89)); // work with the average price
        er.set(new LastShares(100)); // in 4.2 this is LastShares, 4.3 it is
                                        // LastQty
        er.set(new Price(9.89));
        er.set(new TransactTime(testTime)); // default to now
        er.set(new ExecBroker("MVF"));
        er.set(new ExecType(ExecType.FILL)); // PARTIAL_FILL = '1';FILL =
                                                // '2';TRADE = 'F';
        ...
        // er.set(new SettlDate()); // not required for all securities, implied
        ...
        assertEquals(fill.getCumulatedQuantity(), cumulatedQuantity,
            "Failed to convert CumulatedQuantity");
    ...
    public void testFixFillInsert () throws Exception
    {
        ...
        // readMessagesFromFile("TestExecutions.txt");
        ...
        for (String rawMessage : fileMessages)
        {
            ...
            // check the transaction date with the trade date -- should really
            // be the same
            ...
            {
                // System.err.println("Changing TradeDate to date of
            ...
            {
                assertEquals(fill.getOrderQuantity(), fill.getCumulatedQuantity()
                    + fill.getLeavesQuantity(), "Partially filled order quantities do not match");
