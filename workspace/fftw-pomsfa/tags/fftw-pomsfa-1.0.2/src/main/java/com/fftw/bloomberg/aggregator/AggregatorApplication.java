...
public class AggregatorApplication extends MessageCracker implements Application
{
        ...
        TradingPlatform platform) throws FieldNotFound
    {
        ...
        if (message.isSetField(SecurityType.FIELD))
        {
            ...
            else if (SecurityType.OPTION.equals(securityType.getValue()))
            {
                ...
                String symbol = Fix2CmfUtil.optionsSymbol(extractSymbol(message), exchange,
    ...
    /**
     * Not all provides send the symbols in upper-case.  Extract the symbol, uppercase it
     * and then return the symbol.
     * 
     * @param message
     * @return
     * @throws FieldNotFound
     */
    private Symbol extractSymbol(quickfix.fix42.ExecutionReport message) throws FieldNotFound
    {
        Symbol symbol = message.get(new Symbol());
        
        return new Symbol(symbol.getValue().toUpperCase());
    }
