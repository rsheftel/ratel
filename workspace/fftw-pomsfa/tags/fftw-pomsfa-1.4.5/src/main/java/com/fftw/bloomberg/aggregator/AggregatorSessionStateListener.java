...
public class AggregatorSessionStateListener implements SessionStateListener
{
    ...
    public void setLogoutSent (boolean newValue)
    {
        logoutSent = newValue;
    }
    ...
    /**
     * This will check to ensure that we are not disconnected by error.
     * 
    ...
    public void onDisconnect ()
    {
        ...
        if (isThreadChecking() || logoutSent)
        {
            return;
        }
        ...
        Thread timerThread = new Thread(new Runnable()
        {
            public void run ()
            {
                // limit the emails when we are starting a connection
                // some counter-parties force us to logoff when we first logon
                // and we then send out an email - the amount of emails is
                // annoying
                long startTime = getStartTime(mySession);

                boolean emailSent = false;
                while (!withinTimeLimit(startTime, MINUTE) && !mySession.isLogoutReceived()
                    && mySession.isSessionTime() && !isConnected() && !isWeekend(new DateTime()))
                {
            ...
            private long getStartTime (Session mySession)
            {
                try
                {
                    return mySession.getStartTime().getTime();
                }
                catch (IOException e)
                {
    ...
    {
        if (!isWeekend(new DateTime()))
        {
            StringBuilder sb = new StringBuilder(1024);
            sb.append(sessionId).append(" Refreshing session");
            mailer.emailErrorMessage(sb.toString(), "Session refreshed", true);
        }
    }
    ...
    public boolean withinTimeLimit (long date, long limit)
    {
        long currentTime = System.currentTimeMillis();
        long limitTime = date + limit;
