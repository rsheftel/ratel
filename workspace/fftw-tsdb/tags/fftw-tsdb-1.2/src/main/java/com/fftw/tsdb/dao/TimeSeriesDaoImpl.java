...
public class TimeSeriesDaoImpl extends GenericDaoImpl<TimeSeries, Long> implements TimeSeriesDao
{
    ...
    /*
     * public void addTimeSeriesData (TimeSeries timeSeries, TimeSeriesData
     * timeSeriesData, Date observationTime, Double value, String
     * dataSourceName) { timeSeries = em.getReference(TimeSeries.class,
     * timeSeries.getId()); timeSeries.addTimeSeriesData(timeSeriesData);
     * persist(timeSeries); }
     * 
     * public void addTimeSeriesDatasBulk (TimeSeries timeSeries, List<TimeSeriesData>
     * timeSeriesDatas) { if (logger.isDebugEnabled()) {
     * logger.debug("addTimeSeriesDatasBulk is called"); } timeSeries =
     * em.getReference(TimeSeries.class, timeSeries.getId()); for (int i = 0; i <
     * timeSeriesDatas.size(); i++) {
     * timeSeries.addTimeSeriesData(timeSeriesDatas.get(i)); save(timeSeries);
     * if (i % 20 == 0) { if (logger.isDebugEnabled()) { logger.debug("flushing
     * and clearing in memory data. Counter i is: " + i); } flush(); clear(); } } }
     */
        ...
        Date startDate, Date endDate)
    {
        if (logger.isDebugEnabled())
        {
            logger.debug("findDataByDateRange called with time series - " + timeSeriesName
                + " ,data source name - " + dataSourceName + " and date range - " + startDate
                + " to " + endDate);
        }
        List<TimeSeriesData> timeSeriesDatas = (List<TimeSeriesData>)em.createNamedQuery(
            "TimeSeriesData.findDataByDateRange").setParameter("name", dataSourceName)
            .setParameter("timeSeriesName", timeSeriesName).setParameter("startDate", startDate)
