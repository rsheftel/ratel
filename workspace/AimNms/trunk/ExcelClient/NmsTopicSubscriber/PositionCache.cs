...
namespace NmsRtdClient
{
    public class PositionCache
    {
        ...
        public readonly IDictionary batchCache = new Hashtable();
        public readonly IDictionary onlineCache = new Hashtable();
...
//        }
        ...
        private int FindAllFieldValues(string startsWith, ICollection<string> fieldNames, IDictionary cache, IDictionary<string, string[]> book)
        {
             var count = 0;
             lock (lockObject) {
                foreach (DictionaryEntry entry in cache) {
