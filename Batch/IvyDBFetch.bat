...
echo get V2.5/IVYDB.%$DateString%D.zip "%$dest%\IVYDB.%$DateString%D.zip" >> ftp.tmp
echo quit >> ftp.tmp
ftp -s:ftp.tmp -d -n ftp.ivydb.com > c:\systematic\batch\log\IvyDBFetch.%$yyyymmdd%%$hhmm%.log

echo user %$user% %$password% > ftp.tmp
echo dir V2.5/IVYDB.%$DateString%D.zip "%$ldest%\fileStats.txt" >> ftp.tmp
echo quit >> ftp.tmp
ftp -s:ftp.tmp -d -n ftp.ivydb.com >> c:\systematic\batch\log\IvyDBFetch.%$yyyymmdd%%$hhmm%.log
goto :eof
